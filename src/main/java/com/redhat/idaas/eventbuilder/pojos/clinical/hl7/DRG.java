package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class DRG {
    private String DRG_1_DiagnosticRelatedGroup;
    private String DRG_2_DRGAssignedDateTime;
    private String DRG_3_DRGApprovalIndicator;
    private String DRG_4_DRGGrouperReviewCode;
    private String DRG_5_OutlierType;
    private String DRG_6_OutlierDays;
    private String DRG_7_OutlierCost;
    private String DRG_8_DRGPayor;
    private String DRG_9_OutlierReimbursement;
    private String DRG_10_ConfidentialIndicator;
    private String DRG_11_DRGTransferType;
    private String DRG_12_NameofCoder;
    private String DRG_13_GrouperStatus;
    private String DRG_14_PCCLValueCode;
    private String DRG_15_EffectiveWeight;
    private String DRG_16_MonetaryAmount;
    private String DRG_17_StatusPatient;
    private String DRG_18_GrouperSoftwareName;
    private String DRG_19_GrouperSoftwareVersion;
    private String DRG_20_StatusFinancialCalculation;
    private String DRG_21_RelativeDiscountSurcharge;
    private String DRG_22_BasicCharge;
    private String DRG_23_TotalCharge;
    private String DRG_24_DiscountSurcharge;
    private String DRG_25_CalculatedDays;
    private String DRG_26_StatusGender;
    private String DRG_27_StatusAge;
    private String DRG_28_StatusLengthofStay;
    private String DRG_29_StatusSameDayFlag;
    private String DRG_30_StatusSeparationMode;
    private String DRG_31_StatusWeightatBirth;
    private String DRG_32_StatusRespirationMinutes;
    private String DRG_33_StatusAdmission;

    public String getDRG_1_DiagnosticRelatedGroup() {
        return this.DRG_1_DiagnosticRelatedGroup;
    }

    public void setDRG_1_DiagnosticRelatedGroup(final String DRG_1_DiagnosticRelatedGroup) {
        this.DRG_1_DiagnosticRelatedGroup = DRG_1_DiagnosticRelatedGroup;
    }

    public String getDRG_2_DRGAssignedDateTime() {
        return this.DRG_2_DRGAssignedDateTime;
    }

    public void setDRG_2_DRGAssignedDateTime(final String DRG_2_DRGAssignedDateTime) {
        this.DRG_2_DRGAssignedDateTime = DRG_2_DRGAssignedDateTime;
    }

    public String getDRG_3_DRGApprovalIndicator() {
        return this.DRG_3_DRGApprovalIndicator;
    }

    public void setDRG_3_DRGApprovalIndicator(final String DRG_3_DRGApprovalIndicator) {
        this.DRG_3_DRGApprovalIndicator = DRG_3_DRGApprovalIndicator;
    }

    public String getDRG_4_DRGGrouperReviewCode() {
        return this.DRG_4_DRGGrouperReviewCode;
    }

    public void setDRG_4_DRGGrouperReviewCode(final String DRG_4_DRGGrouperReviewCode) {
        this.DRG_4_DRGGrouperReviewCode = DRG_4_DRGGrouperReviewCode;
    }

    public String getDRG_5_OutlierType() {
        return this.DRG_5_OutlierType;
    }

    public void setDRG_5_OutlierType(final String DRG_5_OutlierType) {
        this.DRG_5_OutlierType = DRG_5_OutlierType;
    }

    public String getDRG_6_OutlierDays() {
        return this.DRG_6_OutlierDays;
    }

    public void setDRG_6_OutlierDays(final String DRG_6_OutlierDays) {
        this.DRG_6_OutlierDays = DRG_6_OutlierDays;
    }

    public String getDRG_7_OutlierCost() {
        return this.DRG_7_OutlierCost;
    }

    public void setDRG_7_OutlierCost(final String DRG_7_OutlierCost) {
        this.DRG_7_OutlierCost = DRG_7_OutlierCost;
    }

    public String getDRG_8_DRGPayor() {
        return this.DRG_8_DRGPayor;
    }

    public void setDRG_8_DRGPayor(final String DRG_8_DRGPayor) {
        this.DRG_8_DRGPayor = DRG_8_DRGPayor;
    }

    public String getDRG_9_OutlierReimbursement() {
        return this.DRG_9_OutlierReimbursement;
    }

    public void setDRG_9_OutlierReimbursement(final String DRG_9_OutlierReimbursement) {
        this.DRG_9_OutlierReimbursement = DRG_9_OutlierReimbursement;
    }

    public String getDRG_10_ConfidentialIndicator() {
        return this.DRG_10_ConfidentialIndicator;
    }

    public void setDRG_10_ConfidentialIndicator(final String DRG_10_ConfidentialIndicator) {
        this.DRG_10_ConfidentialIndicator = DRG_10_ConfidentialIndicator;
    }

    public String getDRG_11_DRGTransferType() {
        return this.DRG_11_DRGTransferType;
    }

    public void setDRG_11_DRGTransferType(final String DRG_11_DRGTransferType) {
        this.DRG_11_DRGTransferType = DRG_11_DRGTransferType;
    }

    public String getDRG_12_NameofCoder() {
        return this.DRG_12_NameofCoder;
    }

    public void setDRG_12_NameofCoder(final String DRG_12_NameofCoder) {
        this.DRG_12_NameofCoder = DRG_12_NameofCoder;
    }

    public String getDRG_13_GrouperStatus() {
        return this.DRG_13_GrouperStatus;
    }

    public void setDRG_13_GrouperStatus(final String DRG_13_GrouperStatus) {
        this.DRG_13_GrouperStatus = DRG_13_GrouperStatus;
    }

    public String getDRG_14_PCCLValueCode() {
        return this.DRG_14_PCCLValueCode;
    }

    public void setDRG_14_PCCLValueCode(final String DRG_14_PCCLValueCode) {
        this.DRG_14_PCCLValueCode = DRG_14_PCCLValueCode;
    }

    public String getDRG_15_EffectiveWeight() {
        return this.DRG_15_EffectiveWeight;
    }

    public void setDRG_15_EffectiveWeight(final String DRG_15_EffectiveWeight) {
        this.DRG_15_EffectiveWeight = DRG_15_EffectiveWeight;
    }

    public String getDRG_16_MonetaryAmount() {
        return this.DRG_16_MonetaryAmount;
    }

    public void setDRG_16_MonetaryAmount(final String DRG_16_MonetaryAmount) {
        this.DRG_16_MonetaryAmount = DRG_16_MonetaryAmount;
    }

    public String getDRG_17_StatusPatient() {
        return this.DRG_17_StatusPatient;
    }

    public void setDRG_17_StatusPatient(final String DRG_17_StatusPatient) {
        this.DRG_17_StatusPatient = DRG_17_StatusPatient;
    }

    public String getDRG_18_GrouperSoftwareName() {
        return this.DRG_18_GrouperSoftwareName;
    }

    public void setDRG_18_GrouperSoftwareName(final String DRG_18_GrouperSoftwareName) {
        this.DRG_18_GrouperSoftwareName = DRG_18_GrouperSoftwareName;
    }

    public String getDRG_19_GrouperSoftwareVersion() {
        return this.DRG_19_GrouperSoftwareVersion;
    }

    public void setDRG_19_GrouperSoftwareVersion(final String DRG_19_GrouperSoftwareVersion) {
        this.DRG_19_GrouperSoftwareVersion = DRG_19_GrouperSoftwareVersion;
    }

    public String getDRG_20_StatusFinancialCalculation() {
        return this.DRG_20_StatusFinancialCalculation;
    }

    public void setDRG_20_StatusFinancialCalculation(final String DRG_20_StatusFinancialCalculation) {
        this.DRG_20_StatusFinancialCalculation = DRG_20_StatusFinancialCalculation;
    }

    public String getDRG_21_RelativeDiscountSurcharge() {
        return this.DRG_21_RelativeDiscountSurcharge;
    }

    public void setDRG_21_RelativeDiscountSurcharge(final String DRG_21_RelativeDiscountSurcharge) {
        this.DRG_21_RelativeDiscountSurcharge = DRG_21_RelativeDiscountSurcharge;
    }

    public String getDRG_22_BasicCharge() {
        return this.DRG_22_BasicCharge;
    }

    public void setDRG_22_BasicCharge(final String DRG_22_BasicCharge) {
        this.DRG_22_BasicCharge = DRG_22_BasicCharge;
    }

    public String getDRG_23_TotalCharge() {
        return this.DRG_23_TotalCharge;
    }

    public void setDRG_23_TotalCharge(final String DRG_23_TotalCharge) {
        this.DRG_23_TotalCharge = DRG_23_TotalCharge;
    }

    public String getDRG_24_DiscountSurcharge() {
        return this.DRG_24_DiscountSurcharge;
    }

    public void setDRG_24_DiscountSurcharge(final String DRG_24_DiscountSurcharge) {
        this.DRG_24_DiscountSurcharge = DRG_24_DiscountSurcharge;
    }

    public String getDRG_25_CalculatedDays() {
        return this.DRG_25_CalculatedDays;
    }

    public void setDRG_25_CalculatedDays(final String DRG_25_CalculatedDays) {
        this.DRG_25_CalculatedDays = DRG_25_CalculatedDays;
    }

    public String getDRG_26_StatusGender() {
        return this.DRG_26_StatusGender;
    }

    public void setDRG_26_StatusGender(final String DRG_26_StatusGender) {
        this.DRG_26_StatusGender = DRG_26_StatusGender;
    }

    public String getDRG_27_StatusAge() {
        return this.DRG_27_StatusAge;
    }

    public void setDRG_27_StatusAge(final String DRG_27_StatusAge) {
        this.DRG_27_StatusAge = DRG_27_StatusAge;
    }

    public String getDRG_28_StatusLengthofStay() {
        return this.DRG_28_StatusLengthofStay;
    }

    public void setDRG_28_StatusLengthofStay(final String DRG_28_StatusLengthofStay) {
        this.DRG_28_StatusLengthofStay = DRG_28_StatusLengthofStay;
    }

    public String getDRG_29_StatusSameDayFlag() {
        return this.DRG_29_StatusSameDayFlag;
    }

    public void setDRG_29_StatusSameDayFlag(final String DRG_29_StatusSameDayFlag) {
        this.DRG_29_StatusSameDayFlag = DRG_29_StatusSameDayFlag;
    }

    public String getDRG_30_StatusSeparationMode() {
        return this.DRG_30_StatusSeparationMode;
    }

    public void setDRG_30_StatusSeparationMode(final String DRG_30_StatusSeparationMode) {
        this.DRG_30_StatusSeparationMode = DRG_30_StatusSeparationMode;
    }

    public String getDRG_31_StatusWeightatBirth() {
        return this.DRG_31_StatusWeightatBirth;
    }

    public void setDRG_31_StatusWeightatBirth(final String DRG_31_StatusWeightatBirth) {
        this.DRG_31_StatusWeightatBirth = DRG_31_StatusWeightatBirth;
    }

    public String getDRG_32_StatusRespirationMinutes() {
        return this.DRG_32_StatusRespirationMinutes;
    }

    public void setDRG_32_StatusRespirationMinutes(final String DRG_32_StatusRespirationMinutes) {
        this.DRG_32_StatusRespirationMinutes = DRG_32_StatusRespirationMinutes;
    }

    public String getDRG_33_StatusAdmission() {
        return this.DRG_33_StatusAdmission;
    }

    public void setDRG_33_StatusAdmission(final String DRG_33_StatusAdmission) {
        this.DRG_33_StatusAdmission = DRG_33_StatusAdmission;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

