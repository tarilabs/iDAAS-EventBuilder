package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class OBX {
    private String OBX_1_SetIDOBX;
    private String OBX_2_ValueType;
    private String OBX_3_ObservationIdentifier;
    private String OBX_4_ObservationSubID;
    private String OBX_5_ObservationValue;
    private String OBX_6_Units;
    private String OBX_7_ReferencesRange;
    private String OBX_8_InterpretationCodes;
    private String OBX_9_Probability;
    private String OBX_10_NatureofAbnormalTest;
    private String OBX_11_ObservationResultStatus;
    private String OBX_12_EffectiveDateofReferenceRange;
    private String OBX_13_UserDefinedAccessChecks;
    private String OBX_14_DateTimeoftheObservation;
    private String OBX_15_ProducersID;
    private String OBX_16_ResponsibleObserver;
    private String OBX_17_ObservationMethod;
    private String OBX_18_EquipmentInstanceIdentifier;
    private String OBX_19_DateTimeoftheAnalysis;
    private String OBX_20_ObservationSite;
    private String OBX_21_ObservationInstanceIdentifier;
    private String OBX_22_MoodCode;
    private String OBX_23_PerformingOrganizationName;
    private String OBX_24_PerformingOrganizationAddress;
    private String OBX_25_PerformingOrganizationMedicalDirector;
    private String OBX_26_PatientResultsReleaseCategory;
    private String OBX_27_RootCause;
    private String OBX_28_LocalProcessControl;
    private String OBX_29_ObservationType;
    private String OBX_30_ObservationSubType;

    public String getOBX_1_SetIDOBX() {
        return this.OBX_1_SetIDOBX;
    }

    public void setOBX_1_SetIDOBX(final String OBX_1_SetIDOBX) {
        this.OBX_1_SetIDOBX = OBX_1_SetIDOBX;
    }

    public String getOBX_2_ValueType() {
        return this.OBX_2_ValueType;
    }

    public void setOBX_2_ValueType(final String OBX_2_ValueType) {
        this.OBX_2_ValueType = OBX_2_ValueType;
    }

    public String getOBX_3_ObservationIdentifier() {
        return this.OBX_3_ObservationIdentifier;
    }

    public void setOBX_3_ObservationIdentifier(final String OBX_3_ObservationIdentifier) {
        this.OBX_3_ObservationIdentifier = OBX_3_ObservationIdentifier;
    }

    public String getOBX_4_ObservationSubID() {
        return this.OBX_4_ObservationSubID;
    }

    public void setOBX_4_ObservationSubID(final String OBX_4_ObservationSubID) {
        this.OBX_4_ObservationSubID = OBX_4_ObservationSubID;
    }

    public String getOBX_5_ObservationValue() {
        return this.OBX_5_ObservationValue;
    }

    public void setOBX_5_ObservationValue(final String OBX_5_ObservationValue) {
        this.OBX_5_ObservationValue = OBX_5_ObservationValue;
    }

    public String getOBX_6_Units() {
        return this.OBX_6_Units;
    }

    public void setOBX_6_Units(final String OBX_6_Units) {
        this.OBX_6_Units = OBX_6_Units;
    }

    public String getOBX_7_ReferencesRange() {
        return this.OBX_7_ReferencesRange;
    }

    public void setOBX_7_ReferencesRange(final String OBX_7_ReferencesRange) {
        this.OBX_7_ReferencesRange = OBX_7_ReferencesRange;
    }

    public String getOBX_8_InterpretationCodes() {
        return this.OBX_8_InterpretationCodes;
    }

    public void setOBX_8_InterpretationCodes(final String OBX_8_InterpretationCodes) {
        this.OBX_8_InterpretationCodes = OBX_8_InterpretationCodes;
    }

    public String getOBX_9_Probability() {
        return this.OBX_9_Probability;
    }

    public void setOBX_9_Probability(final String OBX_9_Probability) {
        this.OBX_9_Probability = OBX_9_Probability;
    }

    public String getOBX_10_NatureofAbnormalTest() {
        return this.OBX_10_NatureofAbnormalTest;
    }

    public void setOBX_10_NatureofAbnormalTest(final String OBX_10_NatureofAbnormalTest) {
        this.OBX_10_NatureofAbnormalTest = OBX_10_NatureofAbnormalTest;
    }

    public String getOBX_11_ObservationResultStatus() {
        return this.OBX_11_ObservationResultStatus;
    }

    public void setOBX_11_ObservationResultStatus(final String OBX_11_ObservationResultStatus) {
        this.OBX_11_ObservationResultStatus = OBX_11_ObservationResultStatus;
    }

    public String getOBX_12_EffectiveDateofReferenceRange() {
        return this.OBX_12_EffectiveDateofReferenceRange;
    }

    public void setOBX_12_EffectiveDateofReferenceRange(final String OBX_12_EffectiveDateofReferenceRange) {
        this.OBX_12_EffectiveDateofReferenceRange = OBX_12_EffectiveDateofReferenceRange;
    }

    public String getOBX_13_UserDefinedAccessChecks() {
        return this.OBX_13_UserDefinedAccessChecks;
    }

    public void setOBX_13_UserDefinedAccessChecks(final String OBX_13_UserDefinedAccessChecks) {
        this.OBX_13_UserDefinedAccessChecks = OBX_13_UserDefinedAccessChecks;
    }

    public String getOBX_14_DateTimeoftheObservation() {
        return this.OBX_14_DateTimeoftheObservation;
    }

    public void setOBX_14_DateTimeoftheObservation(final String OBX_14_DateTimeoftheObservation) {
        this.OBX_14_DateTimeoftheObservation = OBX_14_DateTimeoftheObservation;
    }

    public String getOBX_15_ProducersID() {
        return this.OBX_15_ProducersID;
    }

    public void setOBX_15_ProducersID(final String OBX_15_ProducersID) {
        this.OBX_15_ProducersID = OBX_15_ProducersID;
    }

    public String getOBX_16_ResponsibleObserver() {
        return this.OBX_16_ResponsibleObserver;
    }

    public void setOBX_16_ResponsibleObserver(final String OBX_16_ResponsibleObserver) {
        this.OBX_16_ResponsibleObserver = OBX_16_ResponsibleObserver;
    }

    public String getOBX_17_ObservationMethod() {
        return this.OBX_17_ObservationMethod;
    }

    public void setOBX_17_ObservationMethod(final String OBX_17_ObservationMethod) {
        this.OBX_17_ObservationMethod = OBX_17_ObservationMethod;
    }

    public String getOBX_18_EquipmentInstanceIdentifier() {
        return this.OBX_18_EquipmentInstanceIdentifier;
    }

    public void setOBX_18_EquipmentInstanceIdentifier(final String OBX_18_EquipmentInstanceIdentifier) {
        this.OBX_18_EquipmentInstanceIdentifier = OBX_18_EquipmentInstanceIdentifier;
    }

    public String getOBX_19_DateTimeoftheAnalysis() {
        return this.OBX_19_DateTimeoftheAnalysis;
    }

    public void setOBX_19_DateTimeoftheAnalysis(final String OBX_19_DateTimeoftheAnalysis) {
        this.OBX_19_DateTimeoftheAnalysis = OBX_19_DateTimeoftheAnalysis;
    }

    public String getOBX_20_ObservationSite() {
        return this.OBX_20_ObservationSite;
    }

    public void setOBX_20_ObservationSite(final String OBX_20_ObservationSite) {
        this.OBX_20_ObservationSite = OBX_20_ObservationSite;
    }

    public String getOBX_21_ObservationInstanceIdentifier() {
        return this.OBX_21_ObservationInstanceIdentifier;
    }

    public void setOBX_21_ObservationInstanceIdentifier(final String OBX_21_ObservationInstanceIdentifier) {
        this.OBX_21_ObservationInstanceIdentifier = OBX_21_ObservationInstanceIdentifier;
    }

    public String getOBX_22_MoodCode() {
        return this.OBX_22_MoodCode;
    }

    public void setOBX_22_MoodCode(final String OBX_22_MoodCode) {
        this.OBX_22_MoodCode = OBX_22_MoodCode;
    }

    public String getOBX_23_PerformingOrganizationName() {
        return this.OBX_23_PerformingOrganizationName;
    }

    public void setOBX_23_PerformingOrganizationName(final String OBX_23_PerformingOrganizationName) {
        this.OBX_23_PerformingOrganizationName = OBX_23_PerformingOrganizationName;
    }

    public String getOBX_24_PerformingOrganizationAddress() {
        return this.OBX_24_PerformingOrganizationAddress;
    }

    public void setOBX_24_PerformingOrganizationAddress(final String OBX_24_PerformingOrganizationAddress) {
        this.OBX_24_PerformingOrganizationAddress = OBX_24_PerformingOrganizationAddress;
    }

    public String getOBX_25_PerformingOrganizationMedicalDirector() {
        return this.OBX_25_PerformingOrganizationMedicalDirector;
    }

    public void setOBX_25_PerformingOrganizationMedicalDirector(final String OBX_25_PerformingOrganizationMedicalDirector) {
        this.OBX_25_PerformingOrganizationMedicalDirector = OBX_25_PerformingOrganizationMedicalDirector;
    }

    public String getOBX_26_PatientResultsReleaseCategory() {
        return this.OBX_26_PatientResultsReleaseCategory;
    }

    public void setOBX_26_PatientResultsReleaseCategory(final String OBX_26_PatientResultsReleaseCategory) {
        this.OBX_26_PatientResultsReleaseCategory = OBX_26_PatientResultsReleaseCategory;
    }

    public String getOBX_27_RootCause() {
        return this.OBX_27_RootCause;
    }

    public void setOBX_27_RootCause(final String OBX_27_RootCause) {
        this.OBX_27_RootCause = OBX_27_RootCause;
    }

    public String getOBX_28_LocalProcessControl() {
        return this.OBX_28_LocalProcessControl;
    }

    public void setOBX_28_LocalProcessControl(final String OBX_28_LocalProcessControl) {
        this.OBX_28_LocalProcessControl = OBX_28_LocalProcessControl;
    }

    public String getOBX_29_ObservationType() {
        return this.OBX_29_ObservationType;
    }

    public void setOBX_29_ObservationType(final String OBX_29_ObservationType) {
        this.OBX_29_ObservationType = OBX_29_ObservationType;
    }

    public String getOBX_30_ObservationSubType() {
        return this.OBX_30_ObservationSubType;
    }

    public void setOBX_30_ObservationSubType(final String OBX_30_ObservationSubType) {
        this.OBX_30_ObservationSubType = OBX_30_ObservationSubType;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

