package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class PV1 {
    private String PV1_1_SetIDPV1;
    private String PV1_2_PatientClass;
    private String PV1_3_AssignedPatientLocation;
    private String PV1_4_AdmissionType;
    private String PV1_5_PreadmitNumber;
    private String PV1_6_PriorPatientLocation;
    private String PV1_7_AttendingDoctor;
    private String PV1_8_ReferringDoctor;
    private String PV1_9_ConsultingDoctor;
    private String PV1_10_HospitalService;
    private String PV1_11_TemporaryLocation;
    private String PV1_12_PreadmitTestIndicator;
    private String PV1_13_ReadmissionIndicator;
    private String PV1_14_AdmitSource;
    private String PV1_15_AmbulatoryStatus;
    private String PV1_16_VIPIndicator;
    private String PV1_17_AdmittingDoctor;
    private String PV1_18_PatientType;
    private String PV1_19_VisitNumber;
    private String PV1_20_FinancialClass;
    private String PV1_21_ChargePriceIndicator;
    private String PV1_22_CourtesyCode;
    private String PV1_23_CreditRating;
    private String PV1_24_ContractCode;
    private String PV1_25_ContractEffectiveDate;
    private String PV1_26_ContractAmount;
    private String PV1_27_ContractPeriod;
    private String PV1_28_InterestCode;
    private String PV1_29_TransfertoBadDebtCode;
    private String PV1_30_TransfertoBadDebtDate;
    private String PV1_31_BadDebtAgencyCode;
    private String PV1_32_BadDebtTransferAmount;
    private String PV1_33_BadDebtRecoveryAmount;
    private String PV1_34_DeleteAccountIndicator;
    private String PV1_35_DeleteAccountDate;
    private String PV1_36_DischargeDisposition;
    private String PV1_37_DischargedtoLocation;
    private String PV1_38_DietType;
    private String PV1_39_ServicingFacility;
    private String PV1_40_BedStatus;
    private String PV1_41_AccountStatus;
    private String PV1_42_PendingLocation;
    private String PV1_43_PriorTemporaryLocation;
    private String PV1_44_AdmitDateTime;
    private String PV1_45_DischargeDateTime;
    private String PV1_46_CurrentPatientBalance;
    private String PV1_47_TotalCharges;
    private String PV1_48_TotalAdjustments;
    private String PV1_49_TotalPayments;
    private String PV1_50_AlternateVisitID;
    private String PV1_51_VisitIndicator;
    private String PV1_52_OtherHealthcareProvider;
    private String PV1_53_ServiceEpisodeDescription;
    private String PV1_54_ServiceEpisodeIdentifier;

    public String getPV1_1_SetIDPV1() {
        return this.PV1_1_SetIDPV1;
    }

    public void setPV1_1_SetIDPV1(final String PV1_1_SetIDPV1) {
        this.PV1_1_SetIDPV1 = PV1_1_SetIDPV1;
    }

    public String getPV1_2_PatientClass() {
        return this.PV1_2_PatientClass;
    }

    public void setPV1_2_PatientClass(final String PV1_2_PatientClass) {
        this.PV1_2_PatientClass = PV1_2_PatientClass;
    }

    public String getPV1_3_AssignedPatientLocation() {
        return this.PV1_3_AssignedPatientLocation;
    }

    public void setPV1_3_AssignedPatientLocation(final String PV1_3_AssignedPatientLocation) {
        this.PV1_3_AssignedPatientLocation = PV1_3_AssignedPatientLocation;
    }

    public String getPV1_4_AdmissionType() {
        return this.PV1_4_AdmissionType;
    }

    public void setPV1_4_AdmissionType(final String PV1_4_AdmissionType) {
        this.PV1_4_AdmissionType = PV1_4_AdmissionType;
    }

    public String getPV1_5_PreadmitNumber() {
        return this.PV1_5_PreadmitNumber;
    }

    public void setPV1_5_PreadmitNumber(final String PV1_5_PreadmitNumber) {
        this.PV1_5_PreadmitNumber = PV1_5_PreadmitNumber;
    }

    public String getPV1_6_PriorPatientLocation() {
        return this.PV1_6_PriorPatientLocation;
    }

    public void setPV1_6_PriorPatientLocation(final String PV1_6_PriorPatientLocation) {
        this.PV1_6_PriorPatientLocation = PV1_6_PriorPatientLocation;
    }

    public String getPV1_7_AttendingDoctor() {
        return this.PV1_7_AttendingDoctor;
    }

    public void setPV1_7_AttendingDoctor(final String PV1_7_AttendingDoctor) {
        this.PV1_7_AttendingDoctor = PV1_7_AttendingDoctor;
    }

    public String getPV1_8_ReferringDoctor() {
        return this.PV1_8_ReferringDoctor;
    }

    public void setPV1_8_ReferringDoctor(final String PV1_8_ReferringDoctor) {
        this.PV1_8_ReferringDoctor = PV1_8_ReferringDoctor;
    }

    public String getPV1_9_ConsultingDoctor() {
        return this.PV1_9_ConsultingDoctor;
    }

    public void setPV1_9_ConsultingDoctor(final String PV1_9_ConsultingDoctor) {
        this.PV1_9_ConsultingDoctor = PV1_9_ConsultingDoctor;
    }

    public String getPV1_10_HospitalService() {
        return this.PV1_10_HospitalService;
    }

    public void setPV1_10_HospitalService(final String PV1_10_HospitalService) {
        this.PV1_10_HospitalService = PV1_10_HospitalService;
    }

    public String getPV1_11_TemporaryLocation() {
        return this.PV1_11_TemporaryLocation;
    }

    public void setPV1_11_TemporaryLocation(final String PV1_11_TemporaryLocation) {
        this.PV1_11_TemporaryLocation = PV1_11_TemporaryLocation;
    }

    public String getPV1_12_PreadmitTestIndicator() {
        return this.PV1_12_PreadmitTestIndicator;
    }

    public void setPV1_12_PreadmitTestIndicator(final String PV1_12_PreadmitTestIndicator) {
        this.PV1_12_PreadmitTestIndicator = PV1_12_PreadmitTestIndicator;
    }

    public String getPV1_13_ReadmissionIndicator() {
        return this.PV1_13_ReadmissionIndicator;
    }

    public void setPV1_13_ReadmissionIndicator(final String PV1_13_ReadmissionIndicator) {
        this.PV1_13_ReadmissionIndicator = PV1_13_ReadmissionIndicator;
    }

    public String getPV1_14_AdmitSource() {
        return this.PV1_14_AdmitSource;
    }

    public void setPV1_14_AdmitSource(final String PV1_14_AdmitSource) {
        this.PV1_14_AdmitSource = PV1_14_AdmitSource;
    }

    public String getPV1_15_AmbulatoryStatus() {
        return this.PV1_15_AmbulatoryStatus;
    }

    public void setPV1_15_AmbulatoryStatus(final String PV1_15_AmbulatoryStatus) {
        this.PV1_15_AmbulatoryStatus = PV1_15_AmbulatoryStatus;
    }

    public String getPV1_16_VIPIndicator() {
        return this.PV1_16_VIPIndicator;
    }

    public void setPV1_16_VIPIndicator(final String PV1_16_VIPIndicator) {
        this.PV1_16_VIPIndicator = PV1_16_VIPIndicator;
    }

    public String getPV1_17_AdmittingDoctor() {
        return this.PV1_17_AdmittingDoctor;
    }

    public void setPV1_17_AdmittingDoctor(final String PV1_17_AdmittingDoctor) {
        this.PV1_17_AdmittingDoctor = PV1_17_AdmittingDoctor;
    }

    public String getPV1_18_PatientType() {
        return this.PV1_18_PatientType;
    }

    public void setPV1_18_PatientType(final String PV1_18_PatientType) {
        this.PV1_18_PatientType = PV1_18_PatientType;
    }

    public String getPV1_19_VisitNumber() {
        return this.PV1_19_VisitNumber;
    }

    public void setPV1_19_VisitNumber(final String PV1_19_VisitNumber) {
        this.PV1_19_VisitNumber = PV1_19_VisitNumber;
    }

    public String getPV1_20_FinancialClass() {
        return this.PV1_20_FinancialClass;
    }

    public void setPV1_20_FinancialClass(final String PV1_20_FinancialClass) {
        this.PV1_20_FinancialClass = PV1_20_FinancialClass;
    }

    public String getPV1_21_ChargePriceIndicator() {
        return this.PV1_21_ChargePriceIndicator;
    }

    public void setPV1_21_ChargePriceIndicator(final String PV1_21_ChargePriceIndicator) {
        this.PV1_21_ChargePriceIndicator = PV1_21_ChargePriceIndicator;
    }

    public String getPV1_22_CourtesyCode() {
        return this.PV1_22_CourtesyCode;
    }

    public void setPV1_22_CourtesyCode(final String PV1_22_CourtesyCode) {
        this.PV1_22_CourtesyCode = PV1_22_CourtesyCode;
    }

    public String getPV1_23_CreditRating() {
        return this.PV1_23_CreditRating;
    }

    public void setPV1_23_CreditRating(final String PV1_23_CreditRating) {
        this.PV1_23_CreditRating = PV1_23_CreditRating;
    }

    public String getPV1_24_ContractCode() {
        return this.PV1_24_ContractCode;
    }

    public void setPV1_24_ContractCode(final String PV1_24_ContractCode) {
        this.PV1_24_ContractCode = PV1_24_ContractCode;
    }

    public String getPV1_25_ContractEffectiveDate() {
        return this.PV1_25_ContractEffectiveDate;
    }

    public void setPV1_25_ContractEffectiveDate(final String PV1_25_ContractEffectiveDate) {
        this.PV1_25_ContractEffectiveDate = PV1_25_ContractEffectiveDate;
    }

    public String getPV1_26_ContractAmount() {
        return this.PV1_26_ContractAmount;
    }

    public void setPV1_26_ContractAmount(final String PV1_26_ContractAmount) {
        this.PV1_26_ContractAmount = PV1_26_ContractAmount;
    }

    public String getPV1_27_ContractPeriod() {
        return this.PV1_27_ContractPeriod;
    }

    public void setPV1_27_ContractPeriod(final String PV1_27_ContractPeriod) {
        this.PV1_27_ContractPeriod = PV1_27_ContractPeriod;
    }

    public String getPV1_28_InterestCode() {
        return this.PV1_28_InterestCode;
    }

    public void setPV1_28_InterestCode(final String PV1_28_InterestCode) {
        this.PV1_28_InterestCode = PV1_28_InterestCode;
    }

    public String getPV1_29_TransfertoBadDebtCode() {
        return this.PV1_29_TransfertoBadDebtCode;
    }

    public void setPV1_29_TransfertoBadDebtCode(final String PV1_29_TransfertoBadDebtCode) {
        this.PV1_29_TransfertoBadDebtCode = PV1_29_TransfertoBadDebtCode;
    }

    public String getPV1_30_TransfertoBadDebtDate() {
        return this.PV1_30_TransfertoBadDebtDate;
    }

    public void setPV1_30_TransfertoBadDebtDate(final String PV1_30_TransfertoBadDebtDate) {
        this.PV1_30_TransfertoBadDebtDate = PV1_30_TransfertoBadDebtDate;
    }

    public String getPV1_31_BadDebtAgencyCode() {
        return this.PV1_31_BadDebtAgencyCode;
    }

    public void setPV1_31_BadDebtAgencyCode(final String PV1_31_BadDebtAgencyCode) {
        this.PV1_31_BadDebtAgencyCode = PV1_31_BadDebtAgencyCode;
    }

    public String getPV1_32_BadDebtTransferAmount() {
        return this.PV1_32_BadDebtTransferAmount;
    }

    public void setPV1_32_BadDebtTransferAmount(final String PV1_32_BadDebtTransferAmount) {
        this.PV1_32_BadDebtTransferAmount = PV1_32_BadDebtTransferAmount;
    }

    public String getPV1_33_BadDebtRecoveryAmount() {
        return this.PV1_33_BadDebtRecoveryAmount;
    }

    public void setPV1_33_BadDebtRecoveryAmount(final String PV1_33_BadDebtRecoveryAmount) {
        this.PV1_33_BadDebtRecoveryAmount = PV1_33_BadDebtRecoveryAmount;
    }

    public String getPV1_34_DeleteAccountIndicator() {
        return this.PV1_34_DeleteAccountIndicator;
    }

    public void setPV1_34_DeleteAccountIndicator(final String PV1_34_DeleteAccountIndicator) {
        this.PV1_34_DeleteAccountIndicator = PV1_34_DeleteAccountIndicator;
    }

    public String getPV1_35_DeleteAccountDate() {
        return this.PV1_35_DeleteAccountDate;
    }

    public void setPV1_35_DeleteAccountDate(final String PV1_35_DeleteAccountDate) {
        this.PV1_35_DeleteAccountDate = PV1_35_DeleteAccountDate;
    }

    public String getPV1_36_DischargeDisposition() {
        return this.PV1_36_DischargeDisposition;
    }

    public void setPV1_36_DischargeDisposition(final String PV1_36_DischargeDisposition) {
        this.PV1_36_DischargeDisposition = PV1_36_DischargeDisposition;
    }

    public String getPV1_37_DischargedtoLocation() {
        return this.PV1_37_DischargedtoLocation;
    }

    public void setPV1_37_DischargedtoLocation(final String PV1_37_DischargedtoLocation) {
        this.PV1_37_DischargedtoLocation = PV1_37_DischargedtoLocation;
    }

    public String getPV1_38_DietType() {
        return this.PV1_38_DietType;
    }

    public void setPV1_38_DietType(final String PV1_38_DietType) {
        this.PV1_38_DietType = PV1_38_DietType;
    }

    public String getPV1_39_ServicingFacility() {
        return this.PV1_39_ServicingFacility;
    }

    public void setPV1_39_ServicingFacility(final String PV1_39_ServicingFacility) {
        this.PV1_39_ServicingFacility = PV1_39_ServicingFacility;
    }

    public String getPV1_40_BedStatus() {
        return this.PV1_40_BedStatus;
    }

    public void setPV1_40_BedStatus(final String PV1_40_BedStatus) {
        this.PV1_40_BedStatus = PV1_40_BedStatus;
    }

    public String getPV1_41_AccountStatus() {
        return this.PV1_41_AccountStatus;
    }

    public void setPV1_41_AccountStatus(final String PV1_41_AccountStatus) {
        this.PV1_41_AccountStatus = PV1_41_AccountStatus;
    }

    public String getPV1_42_PendingLocation() {
        return this.PV1_42_PendingLocation;
    }

    public void setPV1_42_PendingLocation(final String PV1_42_PendingLocation) {
        this.PV1_42_PendingLocation = PV1_42_PendingLocation;
    }

    public String getPV1_43_PriorTemporaryLocation() {
        return this.PV1_43_PriorTemporaryLocation;
    }

    public void setPV1_43_PriorTemporaryLocation(final String PV1_43_PriorTemporaryLocation) {
        this.PV1_43_PriorTemporaryLocation = PV1_43_PriorTemporaryLocation;
    }

    public String getPV1_44_AdmitDateTime() {
        return this.PV1_44_AdmitDateTime;
    }

    public void setPV1_44_AdmitDateTime(final String PV1_44_AdmitDateTime) {
        this.PV1_44_AdmitDateTime = PV1_44_AdmitDateTime;
    }

    public String getPV1_45_DischargeDateTime() {
        return this.PV1_45_DischargeDateTime;
    }

    public void setPV1_45_DischargeDateTime(final String PV1_45_DischargeDateTime) {
        this.PV1_45_DischargeDateTime = PV1_45_DischargeDateTime;
    }

    public String getPV1_46_CurrentPatientBalance() {
        return this.PV1_46_CurrentPatientBalance;
    }

    public void setPV1_46_CurrentPatientBalance(final String PV1_46_CurrentPatientBalance) {
        this.PV1_46_CurrentPatientBalance = PV1_46_CurrentPatientBalance;
    }

    public String getPV1_47_TotalCharges() {
        return this.PV1_47_TotalCharges;
    }

    public void setPV1_47_TotalCharges(final String PV1_47_TotalCharges) {
        this.PV1_47_TotalCharges = PV1_47_TotalCharges;
    }

    public String getPV1_48_TotalAdjustments() {
        return this.PV1_48_TotalAdjustments;
    }

    public void setPV1_48_TotalAdjustments(final String PV1_48_TotalAdjustments) {
        this.PV1_48_TotalAdjustments = PV1_48_TotalAdjustments;
    }

    public String getPV1_49_TotalPayments() {
        return this.PV1_49_TotalPayments;
    }

    public void setPV1_49_TotalPayments(final String PV1_49_TotalPayments) {
        this.PV1_49_TotalPayments = PV1_49_TotalPayments;
    }

    public String getPV1_50_AlternateVisitID() {
        return this.PV1_50_AlternateVisitID;
    }

    public void setPV1_50_AlternateVisitID(final String PV1_50_AlternateVisitID) {
        this.PV1_50_AlternateVisitID = PV1_50_AlternateVisitID;
    }

    public String getPV1_51_VisitIndicator() {
        return this.PV1_51_VisitIndicator;
    }

    public void setPV1_51_VisitIndicator(final String PV1_51_VisitIndicator) {
        this.PV1_51_VisitIndicator = PV1_51_VisitIndicator;
    }

    public String getPV1_52_OtherHealthcareProvider() {
        return this.PV1_52_OtherHealthcareProvider;
    }

    public void setPV1_52_OtherHealthcareProvider(final String PV1_52_OtherHealthcareProvider) {
        this.PV1_52_OtherHealthcareProvider = PV1_52_OtherHealthcareProvider;
    }

    public String getPV1_53_ServiceEpisodeDescription() {
        return this.PV1_53_ServiceEpisodeDescription;
    }

    public void setPV1_53_ServiceEpisodeDescription(final String PV1_53_ServiceEpisodeDescription) {
        this.PV1_53_ServiceEpisodeDescription = PV1_53_ServiceEpisodeDescription;
    }

    public String getPV1_54_ServiceEpisodeIdentifier() {
        return this.PV1_54_ServiceEpisodeIdentifier;
    }

    public void setPV1_54_ServiceEpisodeIdentifier(final String PV1_54_ServiceEpisodeIdentifier) {
        this.PV1_54_ServiceEpisodeIdentifier = PV1_54_ServiceEpisodeIdentifier;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

