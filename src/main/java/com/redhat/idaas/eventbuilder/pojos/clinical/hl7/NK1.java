package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class NK1 {
    private String NK1_1_SetIDNK1;
    private String NK1_2_Name;
    private String NK1_3_Relationship;
    private String NK1_4_Address;
    private String NK1_5_PhoneNumber;
    private String NK1_6_BusinessPhoneNumber;
    private String NK1_7_ContactRole;
    private String NK1_8_StartDate;
    private String NK1_9_EndDate;
    private String NK1_10_NextofKinAssociatedPartiesJobTitle;
    private String NK1_11_NextofKinAssociatedPartiesJobCodeClass;
    private String NK1_12_NextofKinAssociatedPartiesEmployeeNumber;
    private String NK1_13_OrganizationNameNK1;
    private String NK1_14_MaritalStatus;
    private String NK1_15_AdministrativeSex;
    private String NK1_16_DateTimeofBirth;
    private String NK1_17_LivingDependency;
    private String NK1_18_AmbulatoryStatus;
    private String NK1_19_Citizenship;
    private String NK1_20_PrimaryLanguage;
    private String NK1_21_LivingArrangement;
    private String NK1_22_PublicityCode;
    private String NK1_23_ProtectionIndicator;
    private String NK1_24_StudentIndicator;
    private String NK1_25_Religion;
    private String NK1_26_MothersMaidenName;
    private String NK1_27_Nationality;
    private String NK1_28_EthnicGroup;
    private String NK1_29_ContactReason;
    private String NK1_30_ContactPersonsName;
    private String NK1_31_ContactPersonsTelephoneNumber;
    private String NK1_32_ContactPersonsAddress;
    private String NK1_33_NextofKinAssociatedPartysIdentifiers;
    private String NK1_34_JobStatus;
    private String NK1_35_Race;
    private String NK1_36_Handicap;
    private String NK1_37_ContactPersonSocialSecurityNumber;
    private String NK1_38_NextofKinBirthPlace;
    private String NK1_39_VIPIndicator;
    private String NK1_40_NextofKinTelecommunicationInformation;
    private String NK1_41_ContactPersonsTelecommunicationInformation;

    public String getNK1_1_SetIDNK1() {
        return this.NK1_1_SetIDNK1;
    }

    public void setNK1_1_SetIDNK1(final String NK1_1_SetIDNK1) {
        this.NK1_1_SetIDNK1 = NK1_1_SetIDNK1;
    }

    public String getNK1_2_Name() {
        return this.NK1_2_Name;
    }

    public void setNK1_2_Name(final String NK1_2_Name) {
        this.NK1_2_Name = NK1_2_Name;
    }

    public String getNK1_3_Relationship() {
        return this.NK1_3_Relationship;
    }

    public void setNK1_3_Relationship(final String NK1_3_Relationship) {
        this.NK1_3_Relationship = NK1_3_Relationship;
    }

    public String getNK1_4_Address() {
        return this.NK1_4_Address;
    }

    public void setNK1_4_Address(final String NK1_4_Address) {
        this.NK1_4_Address = NK1_4_Address;
    }

    public String getNK1_5_PhoneNumber() {
        return this.NK1_5_PhoneNumber;
    }

    public void setNK1_5_PhoneNumber(final String NK1_5_PhoneNumber) {
        this.NK1_5_PhoneNumber = NK1_5_PhoneNumber;
    }

    public String getNK1_6_BusinessPhoneNumber() {
        return this.NK1_6_BusinessPhoneNumber;
    }

    public void setNK1_6_BusinessPhoneNumber(final String NK1_6_BusinessPhoneNumber) {
        this.NK1_6_BusinessPhoneNumber = NK1_6_BusinessPhoneNumber;
    }

    public String getNK1_7_ContactRole() {
        return this.NK1_7_ContactRole;
    }

    public void setNK1_7_ContactRole(final String NK1_7_ContactRole) {
        this.NK1_7_ContactRole = NK1_7_ContactRole;
    }

    public String getNK1_8_StartDate() {
        return this.NK1_8_StartDate;
    }

    public void setNK1_8_StartDate(final String NK1_8_StartDate) {
        this.NK1_8_StartDate = NK1_8_StartDate;
    }

    public String getNK1_9_EndDate() {
        return this.NK1_9_EndDate;
    }

    public void setNK1_9_EndDate(final String NK1_9_EndDate) {
        this.NK1_9_EndDate = NK1_9_EndDate;
    }

    public String getNK1_10_NextofKinAssociatedPartiesJobTitle() {
        return this.NK1_10_NextofKinAssociatedPartiesJobTitle;
    }

    public void setNK1_10_NextofKinAssociatedPartiesJobTitle(final String NK1_10_NextofKinAssociatedPartiesJobTitle) {
        this.NK1_10_NextofKinAssociatedPartiesJobTitle = NK1_10_NextofKinAssociatedPartiesJobTitle;
    }

    public String getNK1_11_NextofKinAssociatedPartiesJobCodeClass() {
        return this.NK1_11_NextofKinAssociatedPartiesJobCodeClass;
    }

    public void setNK1_11_NextofKinAssociatedPartiesJobCodeClass(final String NK1_11_NextofKinAssociatedPartiesJobCodeClass) {
        this.NK1_11_NextofKinAssociatedPartiesJobCodeClass = NK1_11_NextofKinAssociatedPartiesJobCodeClass;
    }

    public String getNK1_12_NextofKinAssociatedPartiesEmployeeNumber() {
        return this.NK1_12_NextofKinAssociatedPartiesEmployeeNumber;
    }

    public void setNK1_12_NextofKinAssociatedPartiesEmployeeNumber(final String NK1_12_NextofKinAssociatedPartiesEmployeeNumber) {
        this.NK1_12_NextofKinAssociatedPartiesEmployeeNumber = NK1_12_NextofKinAssociatedPartiesEmployeeNumber;
    }

    public String getNK1_13_OrganizationNameNK1() {
        return this.NK1_13_OrganizationNameNK1;
    }

    public void setNK1_13_OrganizationNameNK1(final String NK1_13_OrganizationNameNK1) {
        this.NK1_13_OrganizationNameNK1 = NK1_13_OrganizationNameNK1;
    }

    public String getNK1_14_MaritalStatus() {
        return this.NK1_14_MaritalStatus;
    }

    public void setNK1_14_MaritalStatus(final String NK1_14_MaritalStatus) {
        this.NK1_14_MaritalStatus = NK1_14_MaritalStatus;
    }

    public String getNK1_15_AdministrativeSex() {
        return this.NK1_15_AdministrativeSex;
    }

    public void setNK1_15_AdministrativeSex(final String NK1_15_AdministrativeSex) {
        this.NK1_15_AdministrativeSex = NK1_15_AdministrativeSex;
    }

    public String getNK1_16_DateTimeofBirth() {
        return this.NK1_16_DateTimeofBirth;
    }

    public void setNK1_16_DateTimeofBirth(final String NK1_16_DateTimeofBirth) {
        this.NK1_16_DateTimeofBirth = NK1_16_DateTimeofBirth;
    }

    public String getNK1_17_LivingDependency() {
        return this.NK1_17_LivingDependency;
    }

    public void setNK1_17_LivingDependency(final String NK1_17_LivingDependency) {
        this.NK1_17_LivingDependency = NK1_17_LivingDependency;
    }

    public String getNK1_18_AmbulatoryStatus() {
        return this.NK1_18_AmbulatoryStatus;
    }

    public void setNK1_18_AmbulatoryStatus(final String NK1_18_AmbulatoryStatus) {
        this.NK1_18_AmbulatoryStatus = NK1_18_AmbulatoryStatus;
    }

    public String getNK1_19_Citizenship() {
        return this.NK1_19_Citizenship;
    }

    public void setNK1_19_Citizenship(final String NK1_19_Citizenship) {
        this.NK1_19_Citizenship = NK1_19_Citizenship;
    }

    public String getNK1_20_PrimaryLanguage() {
        return this.NK1_20_PrimaryLanguage;
    }

    public void setNK1_20_PrimaryLanguage(final String NK1_20_PrimaryLanguage) {
        this.NK1_20_PrimaryLanguage = NK1_20_PrimaryLanguage;
    }

    public String getNK1_21_LivingArrangement() {
        return this.NK1_21_LivingArrangement;
    }

    public void setNK1_21_LivingArrangement(final String NK1_21_LivingArrangement) {
        this.NK1_21_LivingArrangement = NK1_21_LivingArrangement;
    }

    public String getNK1_22_PublicityCode() {
        return this.NK1_22_PublicityCode;
    }

    public void setNK1_22_PublicityCode(final String NK1_22_PublicityCode) {
        this.NK1_22_PublicityCode = NK1_22_PublicityCode;
    }

    public String getNK1_23_ProtectionIndicator() {
        return this.NK1_23_ProtectionIndicator;
    }

    public void setNK1_23_ProtectionIndicator(final String NK1_23_ProtectionIndicator) {
        this.NK1_23_ProtectionIndicator = NK1_23_ProtectionIndicator;
    }

    public String getNK1_24_StudentIndicator() {
        return this.NK1_24_StudentIndicator;
    }

    public void setNK1_24_StudentIndicator(final String NK1_24_StudentIndicator) {
        this.NK1_24_StudentIndicator = NK1_24_StudentIndicator;
    }

    public String getNK1_25_Religion() {
        return this.NK1_25_Religion;
    }

    public void setNK1_25_Religion(final String NK1_25_Religion) {
        this.NK1_25_Religion = NK1_25_Religion;
    }

    public String getNK1_26_MothersMaidenName() {
        return this.NK1_26_MothersMaidenName;
    }

    public void setNK1_26_MothersMaidenName(final String NK1_26_MothersMaidenName) {
        this.NK1_26_MothersMaidenName = NK1_26_MothersMaidenName;
    }

    public String getNK1_27_Nationality() {
        return this.NK1_27_Nationality;
    }

    public void setNK1_27_Nationality(final String NK1_27_Nationality) {
        this.NK1_27_Nationality = NK1_27_Nationality;
    }

    public String getNK1_28_EthnicGroup() {
        return this.NK1_28_EthnicGroup;
    }

    public void setNK1_28_EthnicGroup(final String NK1_28_EthnicGroup) {
        this.NK1_28_EthnicGroup = NK1_28_EthnicGroup;
    }

    public String getNK1_29_ContactReason() {
        return this.NK1_29_ContactReason;
    }

    public void setNK1_29_ContactReason(final String NK1_29_ContactReason) {
        this.NK1_29_ContactReason = NK1_29_ContactReason;
    }

    public String getNK1_30_ContactPersonsName() {
        return this.NK1_30_ContactPersonsName;
    }

    public void setNK1_30_ContactPersonsName(final String NK1_30_ContactPersonsName) {
        this.NK1_30_ContactPersonsName = NK1_30_ContactPersonsName;
    }

    public String getNK1_31_ContactPersonsTelephoneNumber() {
        return this.NK1_31_ContactPersonsTelephoneNumber;
    }

    public void setNK1_31_ContactPersonsTelephoneNumber(final String NK1_31_ContactPersonsTelephoneNumber) {
        this.NK1_31_ContactPersonsTelephoneNumber = NK1_31_ContactPersonsTelephoneNumber;
    }

    public String getNK1_32_ContactPersonsAddress() {
        return this.NK1_32_ContactPersonsAddress;
    }

    public void setNK1_32_ContactPersonsAddress(final String NK1_32_ContactPersonsAddress) {
        this.NK1_32_ContactPersonsAddress = NK1_32_ContactPersonsAddress;
    }

    public String getNK1_33_NextofKinAssociatedPartysIdentifiers() {
        return this.NK1_33_NextofKinAssociatedPartysIdentifiers;
    }

    public void setNK1_33_NextofKinAssociatedPartysIdentifiers(final String NK1_33_NextofKinAssociatedPartysIdentifiers) {
        this.NK1_33_NextofKinAssociatedPartysIdentifiers = NK1_33_NextofKinAssociatedPartysIdentifiers;
    }

    public String getNK1_34_JobStatus() {
        return this.NK1_34_JobStatus;
    }

    public void setNK1_34_JobStatus(final String NK1_34_JobStatus) {
        this.NK1_34_JobStatus = NK1_34_JobStatus;
    }

    public String getNK1_35_Race() {
        return this.NK1_35_Race;
    }

    public void setNK1_35_Race(final String NK1_35_Race) {
        this.NK1_35_Race = NK1_35_Race;
    }

    public String getNK1_36_Handicap() {
        return this.NK1_36_Handicap;
    }

    public void setNK1_36_Handicap(final String NK1_36_Handicap) {
        this.NK1_36_Handicap = NK1_36_Handicap;
    }

    public String getNK1_37_ContactPersonSocialSecurityNumber() {
        return this.NK1_37_ContactPersonSocialSecurityNumber;
    }

    public void setNK1_37_ContactPersonSocialSecurityNumber(final String NK1_37_ContactPersonSocialSecurityNumber) {
        this.NK1_37_ContactPersonSocialSecurityNumber = NK1_37_ContactPersonSocialSecurityNumber;
    }

    public String getNK1_38_NextofKinBirthPlace() {
        return this.NK1_38_NextofKinBirthPlace;
    }

    public void setNK1_38_NextofKinBirthPlace(final String NK1_38_NextofKinBirthPlace) {
        this.NK1_38_NextofKinBirthPlace = NK1_38_NextofKinBirthPlace;
    }

    public String getNK1_39_VIPIndicator() {
        return this.NK1_39_VIPIndicator;
    }

    public void setNK1_39_VIPIndicator(final String NK1_39_VIPIndicator) {
        this.NK1_39_VIPIndicator = NK1_39_VIPIndicator;
    }

    public String getNK1_40_NextofKinTelecommunicationInformation() {
        return this.NK1_40_NextofKinTelecommunicationInformation;
    }

    public void setNK1_40_NextofKinTelecommunicationInformation(final String NK1_40_NextofKinTelecommunicationInformation) {
        this.NK1_40_NextofKinTelecommunicationInformation = NK1_40_NextofKinTelecommunicationInformation;
    }

    public String getNK1_41_ContactPersonsTelecommunicationInformation() {
        return this.NK1_41_ContactPersonsTelecommunicationInformation;
    }

    public void setNK1_41_ContactPersonsTelecommunicationInformation(final String NK1_41_ContactPersonsTelecommunicationInformation) {
        this.NK1_41_ContactPersonsTelecommunicationInformation = NK1_41_ContactPersonsTelecommunicationInformation;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

