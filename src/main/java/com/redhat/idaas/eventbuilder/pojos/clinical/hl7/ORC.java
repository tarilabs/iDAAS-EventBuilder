package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class ORC {
    private String ORC_1_OrderControl;
    private String ORC_2_PlacerOrderNumber;
    private String ORC_3_FillerOrderNumber;
    private String ORC_4_PlacerGroupNumber;
    private String ORC_5_OrderStatus;
    private String ORC_6_ResponseFlag;
    private String ORC_7_QuantityTiming;
    private String ORC_8_ParentOrder;
    private String ORC_9_DateTimeofTransaction;
    private String ORC_10_EnteredBy;
    private String ORC_11_VerifiedBy;
    private String ORC_12_OrderingProvider;
    private String ORC_13_EnterersLocation;
    private String ORC_14_CallBackPhoneNumber;
    private String ORC_15_OrderEffectiveDateTime;
    private String ORC_16_OrderControlCodeReason;
    private String ORC_17_EnteringOrganization;
    private String ORC_18_EnteringDevice;
    private String ORC_19_ActionBy;
    private String ORC_20_AdvancedBeneficiaryNoticeCode;
    private String ORC_21_OrderingFacilityName;
    private String ORC_22_OrderingFacilityAddress;
    private String ORC_23_OrderingFacilityPhoneNumber;
    private String ORC_24_OrderingProviderAddress;
    private String ORC_25_OrderStatusModifier;
    private String ORC_26_AdvancedBeneficiaryNoticeOverrideReason;
    private String ORC_27_FillersExpectedAvailabilityDateTime;
    private String ORC_28_ConfidentialityCode;
    private String ORC_29_OrderType;
    private String ORC_30_EntererAuthorizationMode;
    private String ORC_31_ParentUniversalServiceIdentifier;
    private String ORC_32_AdvancedBeneficiaryNoticeDate;
    private String ORC_33_AlternatePlacerOrderNumber;
    private String ORC_34_OrderWorkflowProfile;

    public String getORC_1_OrderControl() {
        return this.ORC_1_OrderControl;
    }

    public void setORC_1_OrderControl(final String ORC_1_OrderControl) {
        this.ORC_1_OrderControl = ORC_1_OrderControl;
    }

    public String getORC_2_PlacerOrderNumber() {
        return this.ORC_2_PlacerOrderNumber;
    }

    public void setORC_2_PlacerOrderNumber(final String ORC_2_PlacerOrderNumber) {
        this.ORC_2_PlacerOrderNumber = ORC_2_PlacerOrderNumber;
    }

    public String getORC_3_FillerOrderNumber() {
        return this.ORC_3_FillerOrderNumber;
    }

    public void setORC_3_FillerOrderNumber(final String ORC_3_FillerOrderNumber) {
        this.ORC_3_FillerOrderNumber = ORC_3_FillerOrderNumber;
    }

    public String getORC_4_PlacerGroupNumber() {
        return this.ORC_4_PlacerGroupNumber;
    }

    public void setORC_4_PlacerGroupNumber(final String ORC_4_PlacerGroupNumber) {
        this.ORC_4_PlacerGroupNumber = ORC_4_PlacerGroupNumber;
    }

    public String getORC_5_OrderStatus() {
        return this.ORC_5_OrderStatus;
    }

    public void setORC_5_OrderStatus(final String ORC_5_OrderStatus) {
        this.ORC_5_OrderStatus = ORC_5_OrderStatus;
    }

    public String getORC_6_ResponseFlag() {
        return this.ORC_6_ResponseFlag;
    }

    public void setORC_6_ResponseFlag(final String ORC_6_ResponseFlag) {
        this.ORC_6_ResponseFlag = ORC_6_ResponseFlag;
    }

    public String getORC_7_QuantityTiming() {
        return this.ORC_7_QuantityTiming;
    }

    public void setORC_7_QuantityTiming(final String ORC_7_QuantityTiming) {
        this.ORC_7_QuantityTiming = ORC_7_QuantityTiming;
    }

    public String getORC_8_ParentOrder() {
        return this.ORC_8_ParentOrder;
    }

    public void setORC_8_ParentOrder(final String ORC_8_ParentOrder) {
        this.ORC_8_ParentOrder = ORC_8_ParentOrder;
    }

    public String getORC_9_DateTimeofTransaction() {
        return this.ORC_9_DateTimeofTransaction;
    }

    public void setORC_9_DateTimeofTransaction(final String ORC_9_DateTimeofTransaction) {
        this.ORC_9_DateTimeofTransaction = ORC_9_DateTimeofTransaction;
    }

    public String getORC_10_EnteredBy() {
        return this.ORC_10_EnteredBy;
    }

    public void setORC_10_EnteredBy(final String ORC_10_EnteredBy) {
        this.ORC_10_EnteredBy = ORC_10_EnteredBy;
    }

    public String getORC_11_VerifiedBy() {
        return this.ORC_11_VerifiedBy;
    }

    public void setORC_11_VerifiedBy(final String ORC_11_VerifiedBy) {
        this.ORC_11_VerifiedBy = ORC_11_VerifiedBy;
    }

    public String getORC_12_OrderingProvider() {
        return this.ORC_12_OrderingProvider;
    }

    public void setORC_12_OrderingProvider(final String ORC_12_OrderingProvider) {
        this.ORC_12_OrderingProvider = ORC_12_OrderingProvider;
    }

    public String getORC_13_EnterersLocation() {
        return this.ORC_13_EnterersLocation;
    }

    public void setORC_13_EnterersLocation(final String ORC_13_EnterersLocation) {
        this.ORC_13_EnterersLocation = ORC_13_EnterersLocation;
    }

    public String getORC_14_CallBackPhoneNumber() {
        return this.ORC_14_CallBackPhoneNumber;
    }

    public void setORC_14_CallBackPhoneNumber(final String ORC_14_CallBackPhoneNumber) {
        this.ORC_14_CallBackPhoneNumber = ORC_14_CallBackPhoneNumber;
    }

    public String getORC_15_OrderEffectiveDateTime() {
        return this.ORC_15_OrderEffectiveDateTime;
    }

    public void setORC_15_OrderEffectiveDateTime(final String ORC_15_OrderEffectiveDateTime) {
        this.ORC_15_OrderEffectiveDateTime = ORC_15_OrderEffectiveDateTime;
    }

    public String getORC_16_OrderControlCodeReason() {
        return this.ORC_16_OrderControlCodeReason;
    }

    public void setORC_16_OrderControlCodeReason(final String ORC_16_OrderControlCodeReason) {
        this.ORC_16_OrderControlCodeReason = ORC_16_OrderControlCodeReason;
    }

    public String getORC_17_EnteringOrganization() {
        return this.ORC_17_EnteringOrganization;
    }

    public void setORC_17_EnteringOrganization(final String ORC_17_EnteringOrganization) {
        this.ORC_17_EnteringOrganization = ORC_17_EnteringOrganization;
    }

    public String getORC_18_EnteringDevice() {
        return this.ORC_18_EnteringDevice;
    }

    public void setORC_18_EnteringDevice(final String ORC_18_EnteringDevice) {
        this.ORC_18_EnteringDevice = ORC_18_EnteringDevice;
    }

    public String getORC_19_ActionBy() {
        return this.ORC_19_ActionBy;
    }

    public void setORC_19_ActionBy(final String ORC_19_ActionBy) {
        this.ORC_19_ActionBy = ORC_19_ActionBy;
    }

    public String getORC_20_AdvancedBeneficiaryNoticeCode() {
        return this.ORC_20_AdvancedBeneficiaryNoticeCode;
    }

    public void setORC_20_AdvancedBeneficiaryNoticeCode(final String ORC_20_AdvancedBeneficiaryNoticeCode) {
        this.ORC_20_AdvancedBeneficiaryNoticeCode = ORC_20_AdvancedBeneficiaryNoticeCode;
    }

    public String getORC_21_OrderingFacilityName() {
        return this.ORC_21_OrderingFacilityName;
    }

    public void setORC_21_OrderingFacilityName(final String ORC_21_OrderingFacilityName) {
        this.ORC_21_OrderingFacilityName = ORC_21_OrderingFacilityName;
    }

    public String getORC_22_OrderingFacilityAddress() {
        return this.ORC_22_OrderingFacilityAddress;
    }

    public void setORC_22_OrderingFacilityAddress(final String ORC_22_OrderingFacilityAddress) {
        this.ORC_22_OrderingFacilityAddress = ORC_22_OrderingFacilityAddress;
    }

    public String getORC_23_OrderingFacilityPhoneNumber() {
        return this.ORC_23_OrderingFacilityPhoneNumber;
    }

    public void setORC_23_OrderingFacilityPhoneNumber(final String ORC_23_OrderingFacilityPhoneNumber) {
        this.ORC_23_OrderingFacilityPhoneNumber = ORC_23_OrderingFacilityPhoneNumber;
    }

    public String getORC_24_OrderingProviderAddress() {
        return this.ORC_24_OrderingProviderAddress;
    }

    public void setORC_24_OrderingProviderAddress(final String ORC_24_OrderingProviderAddress) {
        this.ORC_24_OrderingProviderAddress = ORC_24_OrderingProviderAddress;
    }

    public String getORC_25_OrderStatusModifier() {
        return this.ORC_25_OrderStatusModifier;
    }

    public void setORC_25_OrderStatusModifier(final String ORC_25_OrderStatusModifier) {
        this.ORC_25_OrderStatusModifier = ORC_25_OrderStatusModifier;
    }

    public String getORC_26_AdvancedBeneficiaryNoticeOverrideReason() {
        return this.ORC_26_AdvancedBeneficiaryNoticeOverrideReason;
    }

    public void setORC_26_AdvancedBeneficiaryNoticeOverrideReason(final String ORC_26_AdvancedBeneficiaryNoticeOverrideReason) {
        this.ORC_26_AdvancedBeneficiaryNoticeOverrideReason = ORC_26_AdvancedBeneficiaryNoticeOverrideReason;
    }

    public String getORC_27_FillersExpectedAvailabilityDateTime() {
        return this.ORC_27_FillersExpectedAvailabilityDateTime;
    }

    public void setORC_27_FillersExpectedAvailabilityDateTime(final String ORC_27_FillersExpectedAvailabilityDateTime) {
        this.ORC_27_FillersExpectedAvailabilityDateTime = ORC_27_FillersExpectedAvailabilityDateTime;
    }

    public String getORC_28_ConfidentialityCode() {
        return this.ORC_28_ConfidentialityCode;
    }

    public void setORC_28_ConfidentialityCode(final String ORC_28_ConfidentialityCode) {
        this.ORC_28_ConfidentialityCode = ORC_28_ConfidentialityCode;
    }

    public String getORC_29_OrderType() {
        return this.ORC_29_OrderType;
    }

    public void setORC_29_OrderType(final String ORC_29_OrderType) {
        this.ORC_29_OrderType = ORC_29_OrderType;
    }

    public String getORC_30_EntererAuthorizationMode() {
        return this.ORC_30_EntererAuthorizationMode;
    }

    public void setORC_30_EntererAuthorizationMode(final String ORC_30_EntererAuthorizationMode) {
        this.ORC_30_EntererAuthorizationMode = ORC_30_EntererAuthorizationMode;
    }

    public String getORC_31_ParentUniversalServiceIdentifier() {
        return this.ORC_31_ParentUniversalServiceIdentifier;
    }

    public void setORC_31_ParentUniversalServiceIdentifier(final String ORC_31_ParentUniversalServiceIdentifier) {
        this.ORC_31_ParentUniversalServiceIdentifier = ORC_31_ParentUniversalServiceIdentifier;
    }

    public String getORC_32_AdvancedBeneficiaryNoticeDate() {
        return this.ORC_32_AdvancedBeneficiaryNoticeDate;
    }

    public void setORC_32_AdvancedBeneficiaryNoticeDate(final String ORC_32_AdvancedBeneficiaryNoticeDate) {
        this.ORC_32_AdvancedBeneficiaryNoticeDate = ORC_32_AdvancedBeneficiaryNoticeDate;
    }

    public String getORC_33_AlternatePlacerOrderNumber() {
        return this.ORC_33_AlternatePlacerOrderNumber;
    }

    public void setORC_33_AlternatePlacerOrderNumber(final String ORC_33_AlternatePlacerOrderNumber) {
        this.ORC_33_AlternatePlacerOrderNumber = ORC_33_AlternatePlacerOrderNumber;
    }

    public String getORC_34_OrderWorkflowProfile() {
        return this.ORC_34_OrderWorkflowProfile;
    }

    public void setORC_34_OrderWorkflowProfile(final String ORC_34_OrderWorkflowProfile) {
        this.ORC_34_OrderWorkflowProfile = ORC_34_OrderWorkflowProfile;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

