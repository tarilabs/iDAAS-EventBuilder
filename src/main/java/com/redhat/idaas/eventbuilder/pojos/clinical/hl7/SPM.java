package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class SPM {
    private String SPM_1_SetIDSPM;
    private String SPM_2_SpecimenID;
    private String SPM_3_SpecimenParentIDs;
    private String SPM_4_SpecimenType;
    private String SPM_5_SpecimenTypeModifier;
    private String SPM_6_SpecimenAdditives;
    private String SPM_7_SpecimenCollectionMethod;
    private String SPM_8_SpecimenSourceSite;
    private String SPM_9_SpecimenSourceSiteModifier;
    private String SPM_10_SpecimenCollectionSite;
    private String SPM_11_SpecimenRole;
    private String SPM_12_SpecimenCollectionAmount;
    private String SPM_13_GroupedSpecimenCount;
    private String SPM_14_SpecimenDescription;
    private String SPM_15_SpecimenHandlingCode;
    private String SPM_16_SpecimenRiskCode;
    private String SPM_17_SpecimenCollectionDateTime;
    private String SPM_18_SpecimenReceivedDateTime;
    private String SPM_19_SpecimenExpirationDateTime;
    private String SPM_20_SpecimenAvailability;
    private String SPM_21_SpecimenRejectReason;
    private String SPM_22_SpecimenQuality;
    private String SPM_23_SpecimenAppropriateness;
    private String SPM_24_SpecimenCondition;
    private String SPM_25_SpecimenCurrentQuantity;
    private String SPM_26_NumberofSpecimenContainers;
    private String SPM_27_ContainerType;
    private String SPM_28_ContainerCondition;
    private String SPM_29_SpecimenChildRole;
    private String SPM_30_AccessionID;
    private String SPM_31_OtherSpecimenID;
    private String SPM_32_ShipmentID;

    public String getSPM_1_SetIDSPM() {
        return this.SPM_1_SetIDSPM;
    }

    public void setSPM_1_SetIDSPM(final String SPM_1_SetIDSPM) {
        this.SPM_1_SetIDSPM = SPM_1_SetIDSPM;
    }

    public String getSPM_2_SpecimenID() {
        return this.SPM_2_SpecimenID;
    }

    public void setSPM_2_SpecimenID(final String SPM_2_SpecimenID) {
        this.SPM_2_SpecimenID = SPM_2_SpecimenID;
    }

    public String getSPM_3_SpecimenParentIDs() {
        return this.SPM_3_SpecimenParentIDs;
    }

    public void setSPM_3_SpecimenParentIDs(final String SPM_3_SpecimenParentIDs) {
        this.SPM_3_SpecimenParentIDs = SPM_3_SpecimenParentIDs;
    }

    public String getSPM_4_SpecimenType() {
        return this.SPM_4_SpecimenType;
    }

    public void setSPM_4_SpecimenType(final String SPM_4_SpecimenType) {
        this.SPM_4_SpecimenType = SPM_4_SpecimenType;
    }

    public String getSPM_5_SpecimenTypeModifier() {
        return this.SPM_5_SpecimenTypeModifier;
    }

    public void setSPM_5_SpecimenTypeModifier(final String SPM_5_SpecimenTypeModifier) {
        this.SPM_5_SpecimenTypeModifier = SPM_5_SpecimenTypeModifier;
    }

    public String getSPM_6_SpecimenAdditives() {
        return this.SPM_6_SpecimenAdditives;
    }

    public void setSPM_6_SpecimenAdditives(final String SPM_6_SpecimenAdditives) {
        this.SPM_6_SpecimenAdditives = SPM_6_SpecimenAdditives;
    }

    public String getSPM_7_SpecimenCollectionMethod() {
        return this.SPM_7_SpecimenCollectionMethod;
    }

    public void setSPM_7_SpecimenCollectionMethod(final String SPM_7_SpecimenCollectionMethod) {
        this.SPM_7_SpecimenCollectionMethod = SPM_7_SpecimenCollectionMethod;
    }

    public String getSPM_8_SpecimenSourceSite() {
        return this.SPM_8_SpecimenSourceSite;
    }

    public void setSPM_8_SpecimenSourceSite(final String SPM_8_SpecimenSourceSite) {
        this.SPM_8_SpecimenSourceSite = SPM_8_SpecimenSourceSite;
    }

    public String getSPM_9_SpecimenSourceSiteModifier() {
        return this.SPM_9_SpecimenSourceSiteModifier;
    }

    public void setSPM_9_SpecimenSourceSiteModifier(final String SPM_9_SpecimenSourceSiteModifier) {
        this.SPM_9_SpecimenSourceSiteModifier = SPM_9_SpecimenSourceSiteModifier;
    }

    public String getSPM_10_SpecimenCollectionSite() {
        return this.SPM_10_SpecimenCollectionSite;
    }

    public void setSPM_10_SpecimenCollectionSite(final String SPM_10_SpecimenCollectionSite) {
        this.SPM_10_SpecimenCollectionSite = SPM_10_SpecimenCollectionSite;
    }

    public String getSPM_11_SpecimenRole() {
        return this.SPM_11_SpecimenRole;
    }

    public void setSPM_11_SpecimenRole(final String SPM_11_SpecimenRole) {
        this.SPM_11_SpecimenRole = SPM_11_SpecimenRole;
    }

    public String getSPM_12_SpecimenCollectionAmount() {
        return this.SPM_12_SpecimenCollectionAmount;
    }

    public void setSPM_12_SpecimenCollectionAmount(final String SPM_12_SpecimenCollectionAmount) {
        this.SPM_12_SpecimenCollectionAmount = SPM_12_SpecimenCollectionAmount;
    }

    public String getSPM_13_GroupedSpecimenCount() {
        return this.SPM_13_GroupedSpecimenCount;
    }

    public void setSPM_13_GroupedSpecimenCount(final String SPM_13_GroupedSpecimenCount) {
        this.SPM_13_GroupedSpecimenCount = SPM_13_GroupedSpecimenCount;
    }

    public String getSPM_14_SpecimenDescription() {
        return this.SPM_14_SpecimenDescription;
    }

    public void setSPM_14_SpecimenDescription(final String SPM_14_SpecimenDescription) {
        this.SPM_14_SpecimenDescription = SPM_14_SpecimenDescription;
    }

    public String getSPM_15_SpecimenHandlingCode() {
        return this.SPM_15_SpecimenHandlingCode;
    }

    public void setSPM_15_SpecimenHandlingCode(final String SPM_15_SpecimenHandlingCode) {
        this.SPM_15_SpecimenHandlingCode = SPM_15_SpecimenHandlingCode;
    }

    public String getSPM_16_SpecimenRiskCode() {
        return this.SPM_16_SpecimenRiskCode;
    }

    public void setSPM_16_SpecimenRiskCode(final String SPM_16_SpecimenRiskCode) {
        this.SPM_16_SpecimenRiskCode = SPM_16_SpecimenRiskCode;
    }

    public String getSPM_17_SpecimenCollectionDateTime() {
        return this.SPM_17_SpecimenCollectionDateTime;
    }

    public void setSPM_17_SpecimenCollectionDateTime(final String SPM_17_SpecimenCollectionDateTime) {
        this.SPM_17_SpecimenCollectionDateTime = SPM_17_SpecimenCollectionDateTime;
    }

    public String getSPM_18_SpecimenReceivedDateTime() {
        return this.SPM_18_SpecimenReceivedDateTime;
    }

    public void setSPM_18_SpecimenReceivedDateTime(final String SPM_18_SpecimenReceivedDateTime) {
        this.SPM_18_SpecimenReceivedDateTime = SPM_18_SpecimenReceivedDateTime;
    }

    public String getSPM_19_SpecimenExpirationDateTime() {
        return this.SPM_19_SpecimenExpirationDateTime;
    }

    public void setSPM_19_SpecimenExpirationDateTime(final String SPM_19_SpecimenExpirationDateTime) {
        this.SPM_19_SpecimenExpirationDateTime = SPM_19_SpecimenExpirationDateTime;
    }

    public String getSPM_20_SpecimenAvailability() {
        return this.SPM_20_SpecimenAvailability;
    }

    public void setSPM_20_SpecimenAvailability(final String SPM_20_SpecimenAvailability) {
        this.SPM_20_SpecimenAvailability = SPM_20_SpecimenAvailability;
    }

    public String getSPM_21_SpecimenRejectReason() {
        return this.SPM_21_SpecimenRejectReason;
    }

    public void setSPM_21_SpecimenRejectReason(final String SPM_21_SpecimenRejectReason) {
        this.SPM_21_SpecimenRejectReason = SPM_21_SpecimenRejectReason;
    }

    public String getSPM_22_SpecimenQuality() {
        return this.SPM_22_SpecimenQuality;
    }

    public void setSPM_22_SpecimenQuality(final String SPM_22_SpecimenQuality) {
        this.SPM_22_SpecimenQuality = SPM_22_SpecimenQuality;
    }

    public String getSPM_23_SpecimenAppropriateness() {
        return this.SPM_23_SpecimenAppropriateness;
    }

    public void setSPM_23_SpecimenAppropriateness(final String SPM_23_SpecimenAppropriateness) {
        this.SPM_23_SpecimenAppropriateness = SPM_23_SpecimenAppropriateness;
    }

    public String getSPM_24_SpecimenCondition() {
        return this.SPM_24_SpecimenCondition;
    }

    public void setSPM_24_SpecimenCondition(final String SPM_24_SpecimenCondition) {
        this.SPM_24_SpecimenCondition = SPM_24_SpecimenCondition;
    }

    public String getSPM_25_SpecimenCurrentQuantity() {
        return this.SPM_25_SpecimenCurrentQuantity;
    }

    public void setSPM_25_SpecimenCurrentQuantity(final String SPM_25_SpecimenCurrentQuantity) {
        this.SPM_25_SpecimenCurrentQuantity = SPM_25_SpecimenCurrentQuantity;
    }

    public String getSPM_26_NumberofSpecimenContainers() {
        return this.SPM_26_NumberofSpecimenContainers;
    }

    public void setSPM_26_NumberofSpecimenContainers(final String SPM_26_NumberofSpecimenContainers) {
        this.SPM_26_NumberofSpecimenContainers = SPM_26_NumberofSpecimenContainers;
    }

    public String getSPM_27_ContainerType() {
        return this.SPM_27_ContainerType;
    }

    public void setSPM_27_ContainerType(final String SPM_27_ContainerType) {
        this.SPM_27_ContainerType = SPM_27_ContainerType;
    }

    public String getSPM_28_ContainerCondition() {
        return this.SPM_28_ContainerCondition;
    }

    public void setSPM_28_ContainerCondition(final String SPM_28_ContainerCondition) {
        this.SPM_28_ContainerCondition = SPM_28_ContainerCondition;
    }

    public String getSPM_29_SpecimenChildRole() {
        return this.SPM_29_SpecimenChildRole;
    }

    public void setSPM_29_SpecimenChildRole(final String SPM_29_SpecimenChildRole) {
        this.SPM_29_SpecimenChildRole = SPM_29_SpecimenChildRole;
    }

    public String getSPM_30_AccessionID() {
        return this.SPM_30_AccessionID;
    }

    public void setSPM_30_AccessionID(final String SPM_30_AccessionID) {
        this.SPM_30_AccessionID = SPM_30_AccessionID;
    }

    public String getSPM_31_OtherSpecimenID() {
        return this.SPM_31_OtherSpecimenID;
    }

    public void setSPM_31_OtherSpecimenID(final String SPM_31_OtherSpecimenID) {
        this.SPM_31_OtherSpecimenID = SPM_31_OtherSpecimenID;
    }

    public String getSPM_32_ShipmentID() {
        return this.SPM_32_ShipmentID;
    }

    public void setSPM_32_ShipmentID(final String SPM_32_ShipmentID) {
        this.SPM_32_ShipmentID = SPM_32_ShipmentID;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

