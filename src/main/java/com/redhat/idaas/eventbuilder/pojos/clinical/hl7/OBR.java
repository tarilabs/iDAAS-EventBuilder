package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class OBR {
    private String OBR_1_SetIDOBR;
    private String OBR_2_PlacerOrderNumber;
    private String OBR_3_FillerOrderNumber;
    private String OBR_4_UniversalServiceIdentifier;
    private String OBR_5_Priority;
    private String OBR_6_RequestedDateTime;
    private String OBR_7_ObservationDateTime;
    private String OBR_8_ObservationEndDateTime;
    private String OBR_9_CollectionVolume;
    private String OBR_10_CollectorIdentifier;
    private String OBR_11_SpecimenActionCode;
    private String OBR_12_DangerCode;
    private String OBR_13_RelevantClinicalInformation;
    private String OBR_14_SpecimenReceivedDateTime;
    private String OBR_15_SpecimenSource;
    private String OBR_16_OrderingProvider;
    private String OBR_17_OrderCallbackPhoneNumber;
    private String OBR_18_PlacerField1;
    private String OBR_19_PlacerField2;
    private String OBR_20_FillerField1;
    private String OBR_21_FillerField2;
    private String OBR_22_ResultsRptStatusChngDateTime;
    private String OBR_23_ChargetoPractice;
    private String OBR_24_DiagnosticServSectID;
    private String OBR_25_ResultStatus;
    private String OBR_26_ParentResult;
    private String OBR_27_QuantityTiming;
    private String OBR_28_ResultCopiesTo;
    private String OBR_29_ParentResultsObservationIdentifier;
    private String OBR_30_TransportationMode;
    private String OBR_31_ReasonforStudy;
    private String OBR_32_PrincipalResultInterpreter;
    private String OBR_33_AssistantResultInterpreter;
    private String OBR_34_Technician;
    private String OBR_35_Transcriptionist;
    private String OBR_36_ScheduledDateTime;
    private String OBR_37_NumberofSampleContainers;
    private String OBR_38_TransportLogisticsofCollectedSample;
    private String OBR_39_CollectorsComment;
    private String OBR_40_TransportArrangementResponsibility;
    private String OBR_41_TransportArranged;
    private String OBR_42_EscortRequired;
    private String OBR_43_PlannedPatientTransportComment;
    private String OBR_44_ProcedureCode;
    private String OBR_45_ProcedureCodeModifier;
    private String OBR_46_PlacerSupplementalServiceInformation;
    private String OBR_47_FillerSupplementalServiceInformation;
    private String OBR_48_MedicallyNecessaryDuplicateProcedureReason;
    private String OBR_49_ResultHandling;
    private String OBR_50_ParentUniversalServiceIdentifier;
    private String OBR_51_ObservationGroupID;
    private String OBR_52_ParentObservationGroupID;
    private String OBR_53_AlternatePlacerOrderNumber;
    private String OBR_54_ParentOrder;

    public String getOBR_1_SetIDOBR() {
        return this.OBR_1_SetIDOBR;
    }

    public void setOBR_1_SetIDOBR(final String OBR_1_SetIDOBR) {
        this.OBR_1_SetIDOBR = OBR_1_SetIDOBR;
    }

    public String getOBR_2_PlacerOrderNumber() {
        return this.OBR_2_PlacerOrderNumber;
    }

    public void setOBR_2_PlacerOrderNumber(final String OBR_2_PlacerOrderNumber) {
        this.OBR_2_PlacerOrderNumber = OBR_2_PlacerOrderNumber;
    }

    public String getOBR_3_FillerOrderNumber() {
        return this.OBR_3_FillerOrderNumber;
    }

    public void setOBR_3_FillerOrderNumber(final String OBR_3_FillerOrderNumber) {
        this.OBR_3_FillerOrderNumber = OBR_3_FillerOrderNumber;
    }

    public String getOBR_4_UniversalServiceIdentifier() {
        return this.OBR_4_UniversalServiceIdentifier;
    }

    public void setOBR_4_UniversalServiceIdentifier(final String OBR_4_UniversalServiceIdentifier) {
        this.OBR_4_UniversalServiceIdentifier = OBR_4_UniversalServiceIdentifier;
    }

    public String getOBR_5_Priority() {
        return this.OBR_5_Priority;
    }

    public void setOBR_5_Priority(final String OBR_5_Priority) {
        this.OBR_5_Priority = OBR_5_Priority;
    }

    public String getOBR_6_RequestedDateTime() {
        return this.OBR_6_RequestedDateTime;
    }

    public void setOBR_6_RequestedDateTime(final String OBR_6_RequestedDateTime) {
        this.OBR_6_RequestedDateTime = OBR_6_RequestedDateTime;
    }

    public String getOBR_7_ObservationDateTime() {
        return this.OBR_7_ObservationDateTime;
    }

    public void setOBR_7_ObservationDateTime(final String OBR_7_ObservationDateTime) {
        this.OBR_7_ObservationDateTime = OBR_7_ObservationDateTime;
    }

    public String getOBR_8_ObservationEndDateTime() {
        return this.OBR_8_ObservationEndDateTime;
    }

    public void setOBR_8_ObservationEndDateTime(final String OBR_8_ObservationEndDateTime) {
        this.OBR_8_ObservationEndDateTime = OBR_8_ObservationEndDateTime;
    }

    public String getOBR_9_CollectionVolume() {
        return this.OBR_9_CollectionVolume;
    }

    public void setOBR_9_CollectionVolume(final String OBR_9_CollectionVolume) {
        this.OBR_9_CollectionVolume = OBR_9_CollectionVolume;
    }

    public String getOBR_10_CollectorIdentifier() {
        return this.OBR_10_CollectorIdentifier;
    }

    public void setOBR_10_CollectorIdentifier(final String OBR_10_CollectorIdentifier) {
        this.OBR_10_CollectorIdentifier = OBR_10_CollectorIdentifier;
    }

    public String getOBR_11_SpecimenActionCode() {
        return this.OBR_11_SpecimenActionCode;
    }

    public void setOBR_11_SpecimenActionCode(final String OBR_11_SpecimenActionCode) {
        this.OBR_11_SpecimenActionCode = OBR_11_SpecimenActionCode;
    }

    public String getOBR_12_DangerCode() {
        return this.OBR_12_DangerCode;
    }

    public void setOBR_12_DangerCode(final String OBR_12_DangerCode) {
        this.OBR_12_DangerCode = OBR_12_DangerCode;
    }

    public String getOBR_13_RelevantClinicalInformation() {
        return this.OBR_13_RelevantClinicalInformation;
    }

    public void setOBR_13_RelevantClinicalInformation(final String OBR_13_RelevantClinicalInformation) {
        this.OBR_13_RelevantClinicalInformation = OBR_13_RelevantClinicalInformation;
    }

    public String getOBR_14_SpecimenReceivedDateTime() {
        return this.OBR_14_SpecimenReceivedDateTime;
    }

    public void setOBR_14_SpecimenReceivedDateTime(final String OBR_14_SpecimenReceivedDateTime) {
        this.OBR_14_SpecimenReceivedDateTime = OBR_14_SpecimenReceivedDateTime;
    }

    public String getOBR_15_SpecimenSource() {
        return this.OBR_15_SpecimenSource;
    }

    public void setOBR_15_SpecimenSource(final String OBR_15_SpecimenSource) {
        this.OBR_15_SpecimenSource = OBR_15_SpecimenSource;
    }

    public String getOBR_16_OrderingProvider() {
        return this.OBR_16_OrderingProvider;
    }

    public void setOBR_16_OrderingProvider(final String OBR_16_OrderingProvider) {
        this.OBR_16_OrderingProvider = OBR_16_OrderingProvider;
    }

    public String getOBR_17_OrderCallbackPhoneNumber() {
        return this.OBR_17_OrderCallbackPhoneNumber;
    }

    public void setOBR_17_OrderCallbackPhoneNumber(final String OBR_17_OrderCallbackPhoneNumber) {
        this.OBR_17_OrderCallbackPhoneNumber = OBR_17_OrderCallbackPhoneNumber;
    }

    public String getOBR_18_PlacerField1() {
        return this.OBR_18_PlacerField1;
    }

    public void setOBR_18_PlacerField1(final String OBR_18_PlacerField1) {
        this.OBR_18_PlacerField1 = OBR_18_PlacerField1;
    }

    public String getOBR_19_PlacerField2() {
        return this.OBR_19_PlacerField2;
    }

    public void setOBR_19_PlacerField2(final String OBR_19_PlacerField2) {
        this.OBR_19_PlacerField2 = OBR_19_PlacerField2;
    }

    public String getOBR_20_FillerField1() {
        return this.OBR_20_FillerField1;
    }

    public void setOBR_20_FillerField1(final String OBR_20_FillerField1) {
        this.OBR_20_FillerField1 = OBR_20_FillerField1;
    }

    public String getOBR_21_FillerField2() {
        return this.OBR_21_FillerField2;
    }

    public void setOBR_21_FillerField2(final String OBR_21_FillerField2) {
        this.OBR_21_FillerField2 = OBR_21_FillerField2;
    }

    public String getOBR_22_ResultsRptStatusChngDateTime() {
        return this.OBR_22_ResultsRptStatusChngDateTime;
    }

    public void setOBR_22_ResultsRptStatusChngDateTime(final String OBR_22_ResultsRptStatusChngDateTime) {
        this.OBR_22_ResultsRptStatusChngDateTime = OBR_22_ResultsRptStatusChngDateTime;
    }

    public String getOBR_23_ChargetoPractice() {
        return this.OBR_23_ChargetoPractice;
    }

    public void setOBR_23_ChargetoPractice(final String OBR_23_ChargetoPractice) {
        this.OBR_23_ChargetoPractice = OBR_23_ChargetoPractice;
    }

    public String getOBR_24_DiagnosticServSectID() {
        return this.OBR_24_DiagnosticServSectID;
    }

    public void setOBR_24_DiagnosticServSectID(final String OBR_24_DiagnosticServSectID) {
        this.OBR_24_DiagnosticServSectID = OBR_24_DiagnosticServSectID;
    }

    public String getOBR_25_ResultStatus() {
        return this.OBR_25_ResultStatus;
    }

    public void setOBR_25_ResultStatus(final String OBR_25_ResultStatus) {
        this.OBR_25_ResultStatus = OBR_25_ResultStatus;
    }

    public String getOBR_26_ParentResult() {
        return this.OBR_26_ParentResult;
    }

    public void setOBR_26_ParentResult(final String OBR_26_ParentResult) {
        this.OBR_26_ParentResult = OBR_26_ParentResult;
    }

    public String getOBR_27_QuantityTiming() {
        return this.OBR_27_QuantityTiming;
    }

    public void setOBR_27_QuantityTiming(final String OBR_27_QuantityTiming) {
        this.OBR_27_QuantityTiming = OBR_27_QuantityTiming;
    }

    public String getOBR_28_ResultCopiesTo() {
        return this.OBR_28_ResultCopiesTo;
    }

    public void setOBR_28_ResultCopiesTo(final String OBR_28_ResultCopiesTo) {
        this.OBR_28_ResultCopiesTo = OBR_28_ResultCopiesTo;
    }

    public String getOBR_29_ParentResultsObservationIdentifier() {
        return this.OBR_29_ParentResultsObservationIdentifier;
    }

    public void setOBR_29_ParentResultsObservationIdentifier(final String OBR_29_ParentResultsObservationIdentifier) {
        this.OBR_29_ParentResultsObservationIdentifier = OBR_29_ParentResultsObservationIdentifier;
    }

    public String getOBR_30_TransportationMode() {
        return this.OBR_30_TransportationMode;
    }

    public void setOBR_30_TransportationMode(final String OBR_30_TransportationMode) {
        this.OBR_30_TransportationMode = OBR_30_TransportationMode;
    }

    public String getOBR_31_ReasonforStudy() {
        return this.OBR_31_ReasonforStudy;
    }

    public void setOBR_31_ReasonforStudy(final String OBR_31_ReasonforStudy) {
        this.OBR_31_ReasonforStudy = OBR_31_ReasonforStudy;
    }

    public String getOBR_32_PrincipalResultInterpreter() {
        return this.OBR_32_PrincipalResultInterpreter;
    }

    public void setOBR_32_PrincipalResultInterpreter(final String OBR_32_PrincipalResultInterpreter) {
        this.OBR_32_PrincipalResultInterpreter = OBR_32_PrincipalResultInterpreter;
    }

    public String getOBR_33_AssistantResultInterpreter() {
        return this.OBR_33_AssistantResultInterpreter;
    }

    public void setOBR_33_AssistantResultInterpreter(final String OBR_33_AssistantResultInterpreter) {
        this.OBR_33_AssistantResultInterpreter = OBR_33_AssistantResultInterpreter;
    }

    public String getOBR_34_Technician() {
        return this.OBR_34_Technician;
    }

    public void setOBR_34_Technician(final String OBR_34_Technician) {
        this.OBR_34_Technician = OBR_34_Technician;
    }

    public String getOBR_35_Transcriptionist() {
        return this.OBR_35_Transcriptionist;
    }

    public void setOBR_35_Transcriptionist(final String OBR_35_Transcriptionist) {
        this.OBR_35_Transcriptionist = OBR_35_Transcriptionist;
    }

    public String getOBR_36_ScheduledDateTime() {
        return this.OBR_36_ScheduledDateTime;
    }

    public void setOBR_36_ScheduledDateTime(final String OBR_36_ScheduledDateTime) {
        this.OBR_36_ScheduledDateTime = OBR_36_ScheduledDateTime;
    }

    public String getOBR_37_NumberofSampleContainers() {
        return this.OBR_37_NumberofSampleContainers;
    }

    public void setOBR_37_NumberofSampleContainers(final String OBR_37_NumberofSampleContainers) {
        this.OBR_37_NumberofSampleContainers = OBR_37_NumberofSampleContainers;
    }

    public String getOBR_38_TransportLogisticsofCollectedSample() {
        return this.OBR_38_TransportLogisticsofCollectedSample;
    }

    public void setOBR_38_TransportLogisticsofCollectedSample(final String OBR_38_TransportLogisticsofCollectedSample) {
        this.OBR_38_TransportLogisticsofCollectedSample = OBR_38_TransportLogisticsofCollectedSample;
    }

    public String getOBR_39_CollectorsComment() {
        return this.OBR_39_CollectorsComment;
    }

    public void setOBR_39_CollectorsComment(final String OBR_39_CollectorsComment) {
        this.OBR_39_CollectorsComment = OBR_39_CollectorsComment;
    }

    public String getOBR_40_TransportArrangementResponsibility() {
        return this.OBR_40_TransportArrangementResponsibility;
    }

    public void setOBR_40_TransportArrangementResponsibility(final String OBR_40_TransportArrangementResponsibility) {
        this.OBR_40_TransportArrangementResponsibility = OBR_40_TransportArrangementResponsibility;
    }

    public String getOBR_41_TransportArranged() {
        return this.OBR_41_TransportArranged;
    }

    public void setOBR_41_TransportArranged(final String OBR_41_TransportArranged) {
        this.OBR_41_TransportArranged = OBR_41_TransportArranged;
    }

    public String getOBR_42_EscortRequired() {
        return this.OBR_42_EscortRequired;
    }

    public void setOBR_42_EscortRequired(final String OBR_42_EscortRequired) {
        this.OBR_42_EscortRequired = OBR_42_EscortRequired;
    }

    public String getOBR_43_PlannedPatientTransportComment() {
        return this.OBR_43_PlannedPatientTransportComment;
    }

    public void setOBR_43_PlannedPatientTransportComment(final String OBR_43_PlannedPatientTransportComment) {
        this.OBR_43_PlannedPatientTransportComment = OBR_43_PlannedPatientTransportComment;
    }

    public String getOBR_44_ProcedureCode() {
        return this.OBR_44_ProcedureCode;
    }

    public void setOBR_44_ProcedureCode(final String OBR_44_ProcedureCode) {
        this.OBR_44_ProcedureCode = OBR_44_ProcedureCode;
    }

    public String getOBR_45_ProcedureCodeModifier() {
        return this.OBR_45_ProcedureCodeModifier;
    }

    public void setOBR_45_ProcedureCodeModifier(final String OBR_45_ProcedureCodeModifier) {
        this.OBR_45_ProcedureCodeModifier = OBR_45_ProcedureCodeModifier;
    }

    public String getOBR_46_PlacerSupplementalServiceInformation() {
        return this.OBR_46_PlacerSupplementalServiceInformation;
    }

    public void setOBR_46_PlacerSupplementalServiceInformation(final String OBR_46_PlacerSupplementalServiceInformation) {
        this.OBR_46_PlacerSupplementalServiceInformation = OBR_46_PlacerSupplementalServiceInformation;
    }

    public String getOBR_47_FillerSupplementalServiceInformation() {
        return this.OBR_47_FillerSupplementalServiceInformation;
    }

    public void setOBR_47_FillerSupplementalServiceInformation(final String OBR_47_FillerSupplementalServiceInformation) {
        this.OBR_47_FillerSupplementalServiceInformation = OBR_47_FillerSupplementalServiceInformation;
    }

    public String getOBR_48_MedicallyNecessaryDuplicateProcedureReason() {
        return this.OBR_48_MedicallyNecessaryDuplicateProcedureReason;
    }

    public void setOBR_48_MedicallyNecessaryDuplicateProcedureReason(final String OBR_48_MedicallyNecessaryDuplicateProcedureReason) {
        this.OBR_48_MedicallyNecessaryDuplicateProcedureReason = OBR_48_MedicallyNecessaryDuplicateProcedureReason;
    }

    public String getOBR_49_ResultHandling() {
        return this.OBR_49_ResultHandling;
    }

    public void setOBR_49_ResultHandling(final String OBR_49_ResultHandling) {
        this.OBR_49_ResultHandling = OBR_49_ResultHandling;
    }

    public String getOBR_50_ParentUniversalServiceIdentifier() {
        return this.OBR_50_ParentUniversalServiceIdentifier;
    }

    public void setOBR_50_ParentUniversalServiceIdentifier(final String OBR_50_ParentUniversalServiceIdentifier) {
        this.OBR_50_ParentUniversalServiceIdentifier = OBR_50_ParentUniversalServiceIdentifier;
    }

    public String getOBR_51_ObservationGroupID() {
        return this.OBR_51_ObservationGroupID;
    }

    public void setOBR_51_ObservationGroupID(final String OBR_51_ObservationGroupID) {
        this.OBR_51_ObservationGroupID = OBR_51_ObservationGroupID;
    }

    public String getOBR_52_ParentObservationGroupID() {
        return this.OBR_52_ParentObservationGroupID;
    }

    public void setOBR_52_ParentObservationGroupID(final String OBR_52_ParentObservationGroupID) {
        this.OBR_52_ParentObservationGroupID = OBR_52_ParentObservationGroupID;
    }

    public String getOBR_53_AlternatePlacerOrderNumber() {
        return this.OBR_53_AlternatePlacerOrderNumber;
    }

    public void setOBR_53_AlternatePlacerOrderNumber(final String OBR_53_AlternatePlacerOrderNumber) {
        this.OBR_53_AlternatePlacerOrderNumber = OBR_53_AlternatePlacerOrderNumber;
    }

    public String getOBR_54_ParentOrder() {
        return this.OBR_54_ParentOrder;
    }

    public void setOBR_54_ParentOrder(final String OBR_54_ParentOrder) {
        this.OBR_54_ParentOrder = OBR_54_ParentOrder;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

