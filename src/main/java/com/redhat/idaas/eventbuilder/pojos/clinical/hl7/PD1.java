package com.redhat.idaas.eventbuilder.pojos.clinical.hl7;

import org.apache.commons.lang3.builder.ReflectionToStringBuilder;

public class PD1 {
    private String PD1_1_LivingDependency;
    private String PD1_2_LivingArrangement;
    private String PD1_3_PatientPrimaryFacility;
    private String PD1_4_PatientPrimaryCareProviderNameIDNo;
    private String PD1_5_StudentIndicator;
    private String PD1_6_Handicap;
    private String PD1_7_LivingWillCode;
    private String PD1_8_OrganDonorCode;
    private String PD1_9_SeparateBill;
    private String PD1_10_DuplicatePatient;
    private String PD1_11_PublicityCode;
    private String PD1_12_ProtectionIndicator;
    private String PD1_13_ProtectionIndicatorEffectiveDate;
    private String PD1_14_PlaceofWorship;
    private String PD1_15_AdvanceDirectiveCode;
    private String PD1_16_ImmunizationRegistryStatus;
    private String PD1_17_ImmunizationRegistryStatusEffectiveDate;
    private String PD1_18_PublicityCodeEffectiveDate;
    private String PD1_19_MilitaryBranch;
    private String PD1_20_MilitaryRankGrade;
    private String PD1_21_MilitaryStatus;
    private String PD1_22_AdvanceDirectiveLastVerifiedDate;

    public String getPD1_1_LivingDependency() {
        return this.PD1_1_LivingDependency;
    }

    public void setPD1_1_LivingDependency(final String PD1_1_LivingDependency) {
        this.PD1_1_LivingDependency = PD1_1_LivingDependency;
    }

    public String getPD1_2_LivingArrangement() {
        return this.PD1_2_LivingArrangement;
    }

    public void setPD1_2_LivingArrangement(final String PD1_2_LivingArrangement) {
        this.PD1_2_LivingArrangement = PD1_2_LivingArrangement;
    }

    public String getPD1_3_PatientPrimaryFacility() {
        return this.PD1_3_PatientPrimaryFacility;
    }

    public void setPD1_3_PatientPrimaryFacility(final String PD1_3_PatientPrimaryFacility) {
        this.PD1_3_PatientPrimaryFacility = PD1_3_PatientPrimaryFacility;
    }

    public String getPD1_4_PatientPrimaryCareProviderNameIDNo() {
        return this.PD1_4_PatientPrimaryCareProviderNameIDNo;
    }

    public void setPD1_4_PatientPrimaryCareProviderNameIDNo(final String PD1_4_PatientPrimaryCareProviderNameIDNo) {
        this.PD1_4_PatientPrimaryCareProviderNameIDNo = PD1_4_PatientPrimaryCareProviderNameIDNo;
    }

    public String getPD1_5_StudentIndicator() {
        return this.PD1_5_StudentIndicator;
    }

    public void setPD1_5_StudentIndicator(final String PD1_5_StudentIndicator) {
        this.PD1_5_StudentIndicator = PD1_5_StudentIndicator;
    }

    public String getPD1_6_Handicap() {
        return this.PD1_6_Handicap;
    }

    public void setPD1_6_Handicap(final String PD1_6_Handicap) {
        this.PD1_6_Handicap = PD1_6_Handicap;
    }

    public String getPD1_7_LivingWillCode() {
        return this.PD1_7_LivingWillCode;
    }

    public void setPD1_7_LivingWillCode(final String PD1_7_LivingWillCode) {
        this.PD1_7_LivingWillCode = PD1_7_LivingWillCode;
    }

    public String getPD1_8_OrganDonorCode() {
        return this.PD1_8_OrganDonorCode;
    }

    public void setPD1_8_OrganDonorCode(final String PD1_8_OrganDonorCode) {
        this.PD1_8_OrganDonorCode = PD1_8_OrganDonorCode;
    }

    public String getPD1_9_SeparateBill() {
        return this.PD1_9_SeparateBill;
    }

    public void setPD1_9_SeparateBill(final String PD1_9_SeparateBill) {
        this.PD1_9_SeparateBill = PD1_9_SeparateBill;
    }

    public String getPD1_10_DuplicatePatient() {
        return this.PD1_10_DuplicatePatient;
    }

    public void setPD1_10_DuplicatePatient(final String PD1_10_DuplicatePatient) {
        this.PD1_10_DuplicatePatient = PD1_10_DuplicatePatient;
    }

    public String getPD1_11_PublicityCode() {
        return this.PD1_11_PublicityCode;
    }

    public void setPD1_11_PublicityCode(final String PD1_11_PublicityCode) {
        this.PD1_11_PublicityCode = PD1_11_PublicityCode;
    }

    public String getPD1_12_ProtectionIndicator() {
        return this.PD1_12_ProtectionIndicator;
    }

    public void setPD1_12_ProtectionIndicator(final String PD1_12_ProtectionIndicator) {
        this.PD1_12_ProtectionIndicator = PD1_12_ProtectionIndicator;
    }

    public String getPD1_13_ProtectionIndicatorEffectiveDate() {
        return this.PD1_13_ProtectionIndicatorEffectiveDate;
    }

    public void setPD1_13_ProtectionIndicatorEffectiveDate(final String PD1_13_ProtectionIndicatorEffectiveDate) {
        this.PD1_13_ProtectionIndicatorEffectiveDate = PD1_13_ProtectionIndicatorEffectiveDate;
    }

    public String getPD1_14_PlaceofWorship() {
        return this.PD1_14_PlaceofWorship;
    }

    public void setPD1_14_PlaceofWorship(final String PD1_14_PlaceofWorship) {
        this.PD1_14_PlaceofWorship = PD1_14_PlaceofWorship;
    }

    public String getPD1_15_AdvanceDirectiveCode() {
        return this.PD1_15_AdvanceDirectiveCode;
    }

    public void setPD1_15_AdvanceDirectiveCode(final String PD1_15_AdvanceDirectiveCode) {
        this.PD1_15_AdvanceDirectiveCode = PD1_15_AdvanceDirectiveCode;
    }

    public String getPD1_16_ImmunizationRegistryStatus() {
        return this.PD1_16_ImmunizationRegistryStatus;
    }

    public void setPD1_16_ImmunizationRegistryStatus(final String PD1_16_ImmunizationRegistryStatus) {
        this.PD1_16_ImmunizationRegistryStatus = PD1_16_ImmunizationRegistryStatus;
    }

    public String getPD1_17_ImmunizationRegistryStatusEffectiveDate() {
        return this.PD1_17_ImmunizationRegistryStatusEffectiveDate;
    }

    public void setPD1_17_ImmunizationRegistryStatusEffectiveDate(final String PD1_17_ImmunizationRegistryStatusEffectiveDate) {
        this.PD1_17_ImmunizationRegistryStatusEffectiveDate = PD1_17_ImmunizationRegistryStatusEffectiveDate;
    }

    public String getPD1_18_PublicityCodeEffectiveDate() {
        return this.PD1_18_PublicityCodeEffectiveDate;
    }

    public void setPD1_18_PublicityCodeEffectiveDate(final String PD1_18_PublicityCodeEffectiveDate) {
        this.PD1_18_PublicityCodeEffectiveDate = PD1_18_PublicityCodeEffectiveDate;
    }

    public String getPD1_19_MilitaryBranch() {
        return this.PD1_19_MilitaryBranch;
    }

    public void setPD1_19_MilitaryBranch(final String PD1_19_MilitaryBranch) {
        this.PD1_19_MilitaryBranch = PD1_19_MilitaryBranch;
    }

    public String getPD1_20_MilitaryRankGrade() {
        return this.PD1_20_MilitaryRankGrade;
    }

    public void setPD1_20_MilitaryRankGrade(final String PD1_20_MilitaryRankGrade) {
        this.PD1_20_MilitaryRankGrade = PD1_20_MilitaryRankGrade;
    }

    public String getPD1_21_MilitaryStatus() {
        return this.PD1_21_MilitaryStatus;
    }

    public void setPD1_21_MilitaryStatus(final String PD1_21_MilitaryStatus) {
        this.PD1_21_MilitaryStatus = PD1_21_MilitaryStatus;
    }

    public String getPD1_22_AdvanceDirectiveLastVerifiedDate() {
        return this.PD1_22_AdvanceDirectiveLastVerifiedDate;
    }

    public void setPD1_22_AdvanceDirectiveLastVerifiedDate(final String PD1_22_AdvanceDirectiveLastVerifiedDate) {
        this.PD1_22_AdvanceDirectiveLastVerifiedDate = PD1_22_AdvanceDirectiveLastVerifiedDate;
    }

    public String toString() {
        return ReflectionToStringBuilder.toString(this);
    }
}

